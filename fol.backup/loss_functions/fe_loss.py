...
class FiniteElementLoss(Loss):
    ...
    def ComputeResidualNorm(self,
                            control_vars: jnp.array,
                            full_dof_vector: jnp.array,
                            relative: bool = True) -> jnp.ndarray:
        """
        Compute the norm of the assembled residual vector for a single sample.

        The assembled residual is obtained as the gradient of the total element
        energy (without loss-function exponent) with respect to the DOF vector.
        The norm is evaluated over free (non-Dirichlet) DOFs only.

        This method is designed to be called inside ``jax.vmap`` for batched
        evaluation.

        Args:
            control_vars (jax.numpy.ndarray):
                Single-sample control/parameter vector (flat).
            full_dof_vector (jax.numpy.ndarray):
                Single-sample full DOF vector with Dirichlet values already
                applied.
            relative (bool, optional):
                If ``True`` (default), return the root-mean-square (RMS) of the
                free-DOF residual.  If ``False``, return the L2 norm.

        Returns:
            jax.numpy.ndarray:
                Scalar residual norm for this sample.
        """
        def energy_fn(dofs):
            return jnp.sum(
                self.ComputeElementsEnergies(self.GetParametersVectors(control_vars), dofs)
            )

        residual = jax.grad(energy_fn)(full_dof_vector)
        free_residual = residual[self.non_dirichlet_indices]

        if relative:
            return jnp.sqrt(jnp.mean(free_residual ** 2))
        return jnp.sqrt(jnp.sum(free_residual ** 2))
