...
class TestMechanical3D(unittest.TestCase):
    ...
    def setUp(self):
        # problem setup
        test_name = 'test_mechanical_3D_tetra_poly_nonlin'
        ...
        dirichlet_bc_dict = {"Ux":{"left":0.0},
                ...
                "Uz":{"left":0.0,"right":-0.1}}
        self.mechanical_loss_3d = NeoHookeMechanicalLoss3DTetra("mechanical_loss_3d",loss_settings={"dirichlet_bc_dict":dirichlet_bc_dict,
                                                                                            "material_dict":{"young_modulus":1,"poisson_ratio":0.3}},
                                                                                            fe_mesh=self.fe_mesh)
        fe_setting = {"linear_solver_settings":{"solver":"PETSc-bcgsl"},
                      "nonlinear_solver_settings":{"rel_tol":1e-5,"abs_tol":1e-5,
                                                    "maxiter":5,"load_incr":10}}
        self.nonlinear_fe_solver = FiniteElementNonLinearResidualBasedSolver("nonlin_fe_solver",self.mechanical_loss_3d,fe_setting)
       ...
       # design NN for learning
        ...
        self.fol.Initialize()
        self.nonlinear_fe_solver.Initialize()        
    ...
    def test_compute(self):
        ...
        U_FOL = np.array(self.fol.Predict(self.coeffs_matrix.reshape(-1,1).T)).reshape(-1)
        U_FEM = np.array(self.nonlinear_fe_solver.Solve(self.K_matrix,np.zeros(U_FOL.shape)))
        ...
        if self.debug_mode=="false":
            ...
            self.fe_mesh['U_FOL'] = np.array(U_FOL).reshape((self.fe_mesh.GetNumberOfNodes(),3))
            self.fe_mesh['U_FEM'] = np.array(U_FEM).reshape((self.fe_mesh.GetNumberOfNodes(),3))
