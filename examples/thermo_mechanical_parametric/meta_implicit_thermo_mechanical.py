...
import optax
import numpy as np
from fol.loss_functions.thermo_mechanical import ThermoMechanicalLoss2DQuad
...
thermomech_loss_2d = ThermoMechanicalLoss2DQuad("thermomechanical_loss_2d",loss_settings={"dirichlet_bc_dict":bc_dict,
                                                                            "material_dict":material_dict, "alpha":1.0},
                                                                            fe_mesh=fe_mesh)
...
# freq_labels = np.concatenate(all_labels)

# output_dict = {
#     "coeffs_matrix_all": coeffs_matrix_all,
#     "freq_labels": freq_labels
# }
# with open("fourier_samples_all.pkl", "wb") as f:
...
FOL_TUV = np.array(fol.Predict((K_matrix[test_start_id:test_end_id,:])))  
FOL_TUV = FOL_TUV.reshape((fe_mesh.GetNumberOfNodes(), 3))
fe_mesh['sol_FOL'] = FOL_TUV

plot_mesh_vec_data_thermal_clear(1,[K_matrix[test_start_id:test_end_id,:],FOL_TUV[:,0],FOL_TUV[:,1],FOL_TUV[:,2]],
...
linear_fe_solver = FiniteElementLinearResidualBasedSolver("linear_fe_solver",thermomech_loss_2d,fe_setting)
linear_fe_solver.Initialize()

FE_TUV = np.array(linear_fe_solver.SolveMulti(K_matrix[test_start_id:test_end_id,:].flatten()
                                         ,np.zeros((fe_mesh.GetNumberOfNodes()*output_size))))  #
...
plot_mesh_vec_data_thermal_clear(1,[K_matrix[test_start_id:test_end_id,:],FE_TUV[:,0],FE_TUV[:,1],FE_TUV[:,2]],
                   ["Heterogeneity","Temperature","X-displacement","Y-displacement"],
                   fig_title="Initial condition and FE solution",cmap = "rainbow",
                   file_name=os.path.join(case_dir,"sol_FE.png"))

plot_mesh_vec_data_thermal_clear(1,[K_matrix[test_start_id:test_end_id,:],absolute_error[:,0],absolute_error[:,1],absolute_error[:,2]],
                   ["Heterogeneity","Temperature","X-displacement","Y-displacement"],
                   fig_title="Absolute error",cmap = "jet",
                   file_name=os.path.join(case_dir,"sol_AbsError.png"))

# Stress and heat flux calculation
FOL_UV = FOL_TUV[:,1:]
FE_UV = FE_TUV[:,1:]
FE_T = FE_TUV[:,0]
FOL_T = FOL_TUV[:,0]

FOLstress_at_nodes = GetStressVector2D(thermomech_loss_2d,fe_mesh,E_matrix.flatten(),
                                       FOL_UV.flatten(),FOL_T.flatten(),initial_temp.flatten())
FE_stress_at_nodes = GetStressVector2D(thermomech_loss_2d,fe_mesh,E_matrix.flatten(),
...
plot_mesh_vec_data_thermal_clear(1,[K_matrix[test_start_id].flatten(),FOLstress_at_nodes[:,0],FOLstress_at_nodes[:,1],FOLstress_at_nodes[:,2]],
                   ["Heterogeneity","XX-stress","YY-stress","XY-stress"],
                   fig_title="FOL stress",cmap = "jet",
                   file_name=os.path.join(case_dir,"stress_FOL.png"))

plot_mesh_vec_data_thermal_clear(1,[K_matrix[test_start_id].flatten(),FE_stress_at_nodes[:,0],FE_stress_at_nodes[:,1],FE_stress_at_nodes[:,2]],
                   ["Heterogeneity","XX-stress","YY-stress","XY-stress"],
                   fig_title="FE stress",cmap = "jet",
                   file_name=os.path.join(case_dir,"stress_FE.png"))
plot_mesh_vec_data_thermal_clear(1,[K_matrix[test_start_id].flatten(),absolute_error[:,0],absolute_error[:,1],absolute_error[:,2]],
                   ["Heterogeneity","XX-stress","YY-stress","XY-stress"],
                   fig_title="Absolute error",cmap = "jet",
                   file_name=os.path.join(case_dir,"stress_AbsError.png"))

FOL_heat_flux_at_nodes = GetHeatFluxVector2D(thermomech_loss_2d,fe_mesh,K_matrix.flatten(),FOL_T.flatten())
FE_heat_flux_at_nodes = GetHeatFluxVector2D(thermomech_loss_2d,fe_mesh,K_matrix.flatten(),FE_T.flatten())
...
plot_mesh_vec_data_thermal_clear(1,[K_matrix[test_start_id].flatten(),FOL_heat_flux_at_nodes[:,0],FOL_heat_flux_at_nodes[:,1]],
...
plot_mesh_vec_data_thermal_clear(1,[K_matrix[test_start_id].flatten(),FE_heat_flux_at_nodes[:,0],FE_heat_flux_at_nodes[:,1]],
                   ["Heterogeneity","X-heat flux","Y-heat flux"],
                   fig_title="FE heat flux",cmap = "inferno",
                   file_name=os.path.join(case_dir,"heat_flux_FE.png"))
plot_mesh_vec_data_thermal_clear(1,[K_matrix[test_start_id].flatten(),heat_flux_absolute_error[:,0],heat_flux_absolute_error[:,1]],
                   ["Heterogeneity","X-heat flux","Y-heat flux"],
