...
def main(ifol_num_epochs=10,clean_dir=False):
    ...
    voronoi_control = VoronoiControl2D("voronoi_control",voronoi_control_settings,fe_mesh)
    ...
    if create_random_coefficients:
        ...
        coeffs_matrix,K_matrix = create_random_fourier_samples(voronoi_control,number_of_random_samples)
        export_dict = model_settings.copy()
        export_dict["coeffs_matrix"] = coeffs_matrix
        with open(f'fourier_control_dict.pkl', 'wb') as f:
            pickle.dump(export_dict,f)
    else:
        with open(f'voronoi_control_dict_dual_n100.pkl', 'rb') as f:
    ...
    # create fol
    fol = MetaAlphaMetaImplicitParametricOperatorLearning(name="meta_implicit_fol",control=voronoi_control,
    ...
    with open(f'train_data_dict_voronoi_N_41.pkl', 'rb') as f:
        train_data_dict = pickle.load(f)

    iFOL_UVs = np.array(fol.Predict(train_data_dict["input_matrix"]))
    ...
    plt.imshow(train_data_dict["output_matrix"][0,::2].reshape(41,41))
    # plt.savefig("ifol2.png")
    plt.show()
    plt.imshow(train_data_dict["output_matrix"][49,::2].reshape(41,41))
    # plt.savefig("ifol2.png")
    plt.show()
