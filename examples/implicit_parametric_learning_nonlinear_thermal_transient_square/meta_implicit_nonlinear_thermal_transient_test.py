...
import optax
import numpy as np
from fol.loss_functions.thermal_transient_nonlinear_2D_fe_quad import ThermalTransientLossNonlinear2DQuad
...
working_directory_name = 'meta_learning_nonlinear_thermal_transient'
...
if create_random_coefficients:
    def generate_random_smooth_patterns(L, N, num_samples=2100):
...
#     FE_T = np.array(nonlinear_fe_solver.Solve(coeffs_matrix[eval_id],coeffs_matrix[eval_id]))  
...
for test in range(eval_start_id,eval_end_id):
    ...
    relative_L2_error = np.zeros(num_steps+1)
    for i in range(num_steps):
        FE_T_temp = np.array(nonlinear_fe_solver.Solve(FE_T_temp,FE_T_temp))  #np.zeros(fe_mesh.GetNumberOfNodes())
        FE_T[:,i] = FE_T_temp    
        relative_L2_error[i+1] = np.linalg.norm(FE_T_temp-FOL_T[:,i])/np.linalg.norm(FE_T_temp)
    # fe_mesh['T_FE'] = FE_T#.reshape((fe_mesh.GetNumberOfNodes(), 1))
    ...
    #                 fig_title="implicit FOL solution and error",cmap = "jet",
    #                 file_name=os.path.join(case_dir,f"FOL-T-dist_test_{eval_id}.png"))
    plot_mesh_vec_data(1,[coeffs_matrix[eval_id],FOL_T[:,4],FOL_T[:,9],FOL_T[:,-1]],
                ["","","",""],
                fig_title="",cmap = "jet",
                file_name=os.path.join(case_dir,f"FOL-dist_test_{eval_id}.png"))
    
    plot_mesh_vec_data(1,[coeffs_matrix[eval_id],FE_T[:,4],FE_T[:,9],FE_T[:,-1]],
                ["","","",""],
                fig_title="",cmap = "jet",
                file_name=os.path.join(case_dir,f"FE-dist_test_{eval_id}.png"))
    plot_mesh_vec_data(1,[coeffs_matrix[eval_id],absolute_error[:,4],absolute_error[:,9],absolute_error[:,-1]],
                ["","","",""],
                fig_title="",cmap = "jet",
                file_name=os.path.join(case_dir,f"FOL-FE-error-dist_test_{eval_id}.png"))
    
    time_plot = np.linspace(0,material_dict["dt"]*num_steps,num_steps+1)
