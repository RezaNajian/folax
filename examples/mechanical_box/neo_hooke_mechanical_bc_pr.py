...
def main(fol_num_epochs=10,solve_FE=False,clean_dir=False):
    ...
    class MLP(nnx.Module):
        def __init__(self, in_features: int, dmid: int, out_features: int, *, rngs: nnx.Rngs):
            self.dense1 = nnx.Linear(in_features, dmid, rngs=rngs,kernel_init=nnx.initializers.constant(0.01),bias_init=nnx.initializers.constant(0.01))
            self.dense2 = nnx.Linear(dmid, out_features, rngs=rngs,kernel_init=nnx.initializers.constant(0.01),bias_init=nnx.initializers.constant(0.01))
