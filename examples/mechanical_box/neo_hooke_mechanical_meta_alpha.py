...
def main(ifol_num_epochs=10,solve_FE=False,clean_dir=False):
    ...
    working_directory_name = "3D_tetra_nonlin_meta_alpha"
    ...
    train_set_pr = coeffs_matrix

    test_start_id = 8
    test_end_id = 10
    # test_set_pr = K_matrix[test_start_id,test_end_id]
    test_set_pr = coeffs_matrix
        ...
        if solve_FE:
            ...
            try:
                hfe_setting = {"linear_solver_settings":{"solver":"JAX-direct"},
                        "nonlinear_solver_settings":{"rel_tol":1e-8,"abs_tol":1e-8,
                                                        "maxiter":5,"load_incr":1}}
                nonlin_hfe_solver = FiniteElementNonLinearResidualBasedSolver("nonlin_fe_solver",mechanical_loss_3d,hfe_setting)
            ...
            fe_mesh['U_HFE'] = HFE_UVW.reshape((fe_mesh.GetNumberOfNodes(), 3))
            fe_mesh['U_FE'] = FE_UVW.reshape((fe_mesh.GetNumberOfNodes(), 3))
            abs_err = abs(FE_UVW.reshape(-1,1) - FOL_UVW.reshape(-1,1))
            fe_mesh[f"abs_error"] = abs_err.reshape((fe_mesh.GetNumberOfNodes(), 3))
