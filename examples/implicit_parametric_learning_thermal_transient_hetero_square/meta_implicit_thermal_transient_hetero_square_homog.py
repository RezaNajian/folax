...
working_directory_name = 'siren_implicit_thermal_2D_homog2'
...
if create_random_coefficients:
    def generate_random_smooth_patterns(L, N, num_samples=1500):
    ...
    hetero_info = np.full((1,fe_mesh.GetNumberOfNodes()),1.0)#generate_morph_pattern(model_settings["N"]).reshape(1,-1) #
else:
    pass
...
plot_mesh_vec_data(1,[coeffs_matrix[eval_id],FOL_T[:,time_list[0]],FOL_T[:,time_list[1]],FOL_T[:,time_list[2]]],
                   ["","","",""],
                   fig_title="Initial condition and implicit FOL solution",cmap = "jet",
                   file_name=os.path.join(case_dir,"FOL-T-dist.png"))

plot_mesh_vec_data(1,[coeffs_matrix[eval_id],FE_T[:,time_list[0]],FE_T[:,time_list[1]],FE_T[:,time_list[2]]],
                   ["","","",""],
