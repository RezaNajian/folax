...
main_name = 'pi_deep_o_net_thermo_mechanical'
working_directory_name = main_name + '_test'
...
deeponet_learning.RestoreState(restore_state_directory=main_name+'/flax_final_state')
# num_steps = 1
...
for i in np.arange(test_start_id,test_end_id,1):
    FOL_TUVW = np.array(deeponet_learning.Predict((bc_matrix[i:i+1,:].reshape(-1,1))))  
