...
class FourierNeuralOperator2DSeparate(nn.Module):
  r'''
  Fourier Neural Operator for 2D signals.
  ...
  def __call__(self, x:jnp.ndarray) -> jnp.ndarray:
    ...
    # Unpad
    if self.padding > 0:
      x1 = x1[:, :-self.padding, :-self.padding, :]
      x2 = x2[:, :-self.padding, :-self.padding, :]
    # Project to the output channels
...
class FourierNeuralOperator2DFullSeparate(nn.Module):
  r'''
  Fourier Neural Operator for 2D signals.
  ...
  def __call__(self, x:jnp.ndarray) -> jnp.ndarray:
    ...
    if self.padding > 0:
      x1 = x1[:, :-self.padding, :-self.padding, :]
      x2 = x2[:, :-self.padding, :-self.padding, :]
      x3 = x3[:, :-self.padding, :-self.padding, :]
    # Project to the output channels
    x1 = nn.Dense(self.channels_last_proj)(x1)
    x1 = self.activation(x1)
    ...
    return self.output_scale * jnp.concatenate([x1, x2, x3], axis=-1)
