...
from fourier_neural_operator_networks import FourierNeuralOperator2DFullSeparate
...
working_directory_name = 'pi_fno_thermomech_2D_full_sep'
...
fno_model = bridge.ToNNX(FourierNeuralOperator2DFullSeparate(modes1=12,
                                                modes2=12,
                                                width=20,
                                                depth=4,
...
fe_setting = {
    ...
    "nonlinear_solver_settings":{
        "rel_tol":1e-8,"abs_tol":1e-8,"maxiter":10,"load_incr":5}
}
...
batch_dir = os.path.join(case_dir, "batch_5000_50")
...
for sid in sample_ids:
    ...
    # --- FOL prediction ---
    # (Assumes fol.Predict accepts batch; if not, keep the [None, :] batching)
    K_vec = K_matrix[sid, :]
