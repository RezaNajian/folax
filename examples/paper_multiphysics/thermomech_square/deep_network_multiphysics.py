...
class DeepNetwork(ABC):
    ...
    default_convergence_settings = {"num_epochs":100,"convergence_criterion":"total_loss",
                                    "relative_error":1e-8,"absolute_error":1e-8,"staggered":False,"staggered_epochs":25}
    default_plot_settings = {"plot_list":["total_loss"],"plot_frequency":1,"save_frequency":100,"save_directory":".","multiphysics":False}
    ...
    @partial(nnx.jit, static_argnums=(0,))
    def ComputeBatchLossValueMP(self,batch_set:Tuple[jnp.ndarray, jnp.ndarray],nn_model:nnx.Module):
        """
        Computes the loss values for a batch of data.
    ...
    @partial(nnx.jit, static_argnums=(0,))
    def ComputeBatchLossValueMP1(self,batch_set:Tuple[jnp.ndarray, jnp.ndarray],nn_model:nnx.Module):
        """
        Computes the loss values for a batch of data.
        ...
        batch_losses,batch_losses_data= nnx.vmap(self.ComputeSingleLossValueStaggered1,in_axes=(0, None),out_axes=0)(batch_set,nn_model)
        loss_name = self.loss_function.GetName()
        total_mean_loss = jnp.mean(batch_losses)
        loss_data = ({loss_name+"_min":jnp.min(batch_losses_data[0]),
                                         loss_name+"_max":jnp.max(batch_losses_data[1]),
    ...
    @partial(nnx.jit, static_argnums=(0,))
    def ComputeBatchLossValueMP2(self,batch_set:Tuple[jnp.ndarray, jnp.ndarray],nn_model:nnx.Module):
        """
        Computes the loss values for a batch of data.
        ...
        batch_losses,batch_losses_data= nnx.vmap(self.ComputeSingleLossValueStaggered2,in_axes=(0, None),out_axes=0)(batch_set,nn_model)
        loss_name = self.loss_function.GetName()
        total_mean_loss = jnp.mean(batch_losses)
        loss_data = ({loss_name+"_min":jnp.min(batch_losses_data[0]),
                                         loss_name+"_max":jnp.max(batch_losses_data[1]),
    ...
    @partial(nnx.jit, static_argnums=(0,))
    def TrainStepMP(self, state, data):
        nn, opt = state
        (_,batch_dict), batch_grads = nnx.value_and_grad(self.ComputeBatchLossValueMP,argnums=1,has_aux=True) \
                                                                    (data,nn)
        opt.update(nn, batch_grads)
        return (batch_dict["total_loss"], batch_dict["phy1_loss"], batch_dict["phy2_loss"])
    
    @partial(nnx.jit, static_argnums=(0,))
    def TestStepMP(self, state, data):
        nn,_ = state
        (_,batch_dict) = self.ComputeBatchLossValueMP(data,nn)
        return (batch_dict["total_loss"], batch_dict["phy1_loss"], batch_dict["phy2_loss"])
    
    @partial(nnx.jit, static_argnums=(0,))
    def TrainStepMP1(self, state, data):
        nn, opt = state
        (_,batch_dict), batch_grads = nnx.value_and_grad(self.ComputeBatchLossValueMP1,argnums=1,has_aux=True) \
                                                                    (data,nn)
        opt.update(nn,batch_grads)
        return (batch_dict["total_loss"], batch_dict["phy1_loss"], batch_dict["phy2_loss"])
    
    @partial(nnx.jit, static_argnums=(0,))
    def TestStepMP1(self, state, data):
        nn,_ = state
        (_,batch_dict) = self.ComputeBatchLossValueMP1(data,nn)
        return (batch_dict["total_loss"], batch_dict["phy1_loss"], batch_dict["phy2_loss"])
    
    @partial(nnx.jit, static_argnums=(0,))
    def TrainStepMP2(self, state, data):
        nn, opt = state
        (_,batch_dict), batch_grads = nnx.value_and_grad(self.ComputeBatchLossValueMP2,argnums=1,has_aux=True) \
                                                                    (data,nn)
        opt.update(nn,batch_grads)
        return (batch_dict["total_loss"], batch_dict["phy1_loss"], batch_dict["phy2_loss"])
    
    @partial(nnx.jit, static_argnums=(0,))
    def TestStepMP2(self, state, data):
        nn,_ = state
        (_,batch_dict) = self.ComputeBatchLossValueMP2(data,nn)
        return (batch_dict["total_loss"], batch_dict["phy1_loss"], batch_dict["phy2_loss"])   
    ...
    def Train(self, 
        ...
        if convergence_settings["staggered"]:
            if plot_settings["multiphysics"]:
               train_multiple_steps_with_idxs_1 = nnx.jit(lambda st, dat, idxs: nnx.scan(lambda st, idxs: (st, self.TrainStepMP1(st, jax.tree.map(lambda a: a[idxs], dat))))(st, idxs))
               train_multiple_steps_with_idxs_2 = nnx.jit(lambda st, dat, idxs: nnx.scan(lambda st, idxs: (st, self.TrainStepMP2(st, jax.tree.map(lambda a: a[idxs], dat))))(st, idxs))
            else:
                exit("staggered training is only implemented for multiphysics problems")
            for epoch in pbar:
                ...
                if train_checkpoint_settings["least_loss_checkpointing"] and restore_nnx_state_settings['restore'] and epoch==0:
                    if plot_settings["multiphysics"]:
                        train_checkpoint_settings["least_loss"] = self.TestStepMP(state,train_set)[0]
                    else:
                ...
                if test_checkpoint_settings["least_loss_checkpointing"] and restore_nnx_state_settings['restore'] and epoch==0:
                    if plot_settings["multiphysics"]:
                        test_checkpoint_settings["least_loss"] = self.TestStepMP(state,test_set)[0]
                    else:
                ...
                if (epoch // convergence_settings["staggered_epochs"]) % 2 == 0:
                    _, losses = train_multiple_steps_with_idxs_1(state, train_set, order)
                else:
                    _, losses = train_multiple_steps_with_idxs_2(state, train_set, order)
                ...
                if len(test_set[0])>0 and ((epoch)%test_frequency==0 or epoch==convergence_settings["num_epochs"]-1):
                    if plot_settings["multiphysics"]:
                        test_history_dict["total_loss"].append(self.TestStepMP(state,test_set)[0])
                        test_history_dict["phy1_loss"].append(self.TestStepMP(state,test_set)[1])
                        test_history_dict["phy2_loss"].append(self.TestStepMP(state,test_set)[2])
                    else:
            ...
            step_fn = self.TrainStepMP if plot_settings["multiphysics"] else self.TrainStep
            test_fn = self.TestStepMP if plot_settings["multiphysics"] else self.TestStep
            def _gather(dat, idxs):
                return jax.tree.map(lambda a: jnp.take(a, idxs, axis=0), dat)
            # train_multiple_steps_with_idxs = nnx.jit(lambda st, dat, idxs: nnx.scan(lambda st, idxs: (st, step_fn(st, jax.tree.map(lambda a: a[idxs], dat))))(st, idxs))
            train_multiple_steps_with_idxs = nnx.jit(
                lambda st, dat, idxs: nnx.scan(
                    lambda carry, i: (carry, step_fn(carry, _gather(dat, i)))
                )(st, idxs),
                donate_argnums=(0,)  
            )
            ...
            for epoch in pbar:
                ...
                if train_checkpoint_settings["least_loss_checkpointing"] and restore_nnx_state_settings['restore'] and epoch==0:
                    if plot_settings["multiphysics"]:
                        train_checkpoint_settings["least_loss"] = step_fn(state,train_set)[0]
                    else:
                        train_checkpoint_settings["least_loss"] = step_fn(state,train_set)
                if test_checkpoint_settings["least_loss_checkpointing"] and restore_nnx_state_settings['restore'] and epoch==0:
                    if plot_settings["multiphysics"]:
                        test_checkpoint_settings["least_loss"] = step_fn(state,test_set)[0]
                    else:
                        test_checkpoint_settings["least_loss"] = step_fn(state,test_set)
                ...
                if len(test_set[0])>0 and ((epoch)%test_frequency==0 or epoch==convergence_settings["num_epochs"]-1):
                    if plot_settings["multiphysics"]:
                        test_history_dict["total_loss"].append(test_fn(state,test_set)[0])
                        test_history_dict["phy1_loss"].append(test_fn(state,test_set)[1])
                        test_history_dict["phy2_loss"].append(test_fn(state,test_set)[2])
                    else:
                        test_history_dict["total_loss"].append(test_fn(state,test_set))
                # print step   
                if len(test_set[0])>0:
                    print_dict = {"train_loss":train_history_dict["total_loss"][-1],
                                  "test_loss":test_history_dict["total_loss"][-1]}
