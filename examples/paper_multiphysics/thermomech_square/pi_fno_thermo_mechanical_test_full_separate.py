...
working_directory_name = 'pi_fno_thermomech_2D_full_separate_test'
...
# load teh best model
pi_fno_pr_learning.RestoreState(restore_state_directory="pi_fno_thermomech_2D_full_sep/flax_final_state")
...
for sid in sample_ids:
    ...
    plot_mesh_vec_data_hetero(1,[K_vec],["Heterogeneity"],
        ...
        file_name=os.path.join(sample_dir, f"hetero_{sid}.png"),)
    FOL_TUV = np.array(pi_fno_pr_learning.Predict(K_vec[None, :])).reshape(n_nodes, 3)
    fe_mesh['Heterogeneity_K'] = K_vec.reshape(-1,1)
    fe_mesh['sol_FOL'] = FOL_TUV
