...
working_directory_name = main_name + '_test_poly'
case_dir = os.path.join('.', working_directory_name)
create_clean_directory(working_directory_name)
sys.stdout = Logger(os.path.join(case_dir,working_directory_name+".log"))

model_settings = {"L":1,"N":44,
...
if create_random_coefficients:
    ...
    voronoi_control = VoronoiControl3D("first_voronoi_control",voronoi_control_settings,fe_mesh)
    voronoi_control.Initialize()
    _, K_matrix = create_random_voronoi_samples(voronoi_control,number_of_random_samples,dim=3)
...
for i in range(20):    
    K_matrix = K_matrix_all[i:i+1]
    FOL_TUVW = np.array(pi_fno_pr_learning.Predict(K_matrix)) 
    ...
    FE_TUVW = np.array(nonlinear_fe_solver.Solve(K_matrix.flatten(),np.zeros((fe_mesh.GetNumberOfNodes()*4))))  #
    ...
    fe_mesh['Heterogeneity'] = K_matrix[test_start_id:test_end_id,:].reshape((fe_mesh.GetNumberOfNodes(),1))
    ...
    FOLstress_at_nodes = GetStressVector3D(thermomech_loss_3d,fe_mesh, K_matrix[test_start_id:test_end_id,:].flatten(),
                            FOL_UVW.flatten(),FOL_T.flatten(),initial_temp.flatten())
    FE_stress_at_nodes = GetStressVector3D(thermomech_loss_3d,fe_mesh, K_matrix[test_start_id:test_end_id,:].flatten(),
    ...
    FOL_heat_flux_at_nodes = GetHeatFluxVector3D(thermomech_loss_3d,fe_mesh,K_matrix[test_start_id:test_end_id,:].flatten(),FOL_T.flatten())
    FE_heat_flux_at_nodes = GetHeatFluxVector3D(thermomech_loss_3d,fe_mesh,K_matrix[test_start_id:test_end_id,:].flatten(),FE_T.flatten())
