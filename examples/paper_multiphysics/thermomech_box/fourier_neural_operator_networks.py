...
class SpectralConv3d(nn.Module):
  ...
  def __call__(self, x):
    ...
    # Go back to the spatial domain
    y = jnp.fft.irfftn(out_ft, axes=(1, 2, 3))
    return y
...
class FourierNeuralOperator3DSeparate(nn.Module):
  r'''
  Fourier Neural Operator for 2D signals.
  ...
  channels_last_proj: int = 128
  activation: Callable = nn.gelu
  out_channels_1: int = 1
  out_channels_2: int = 1
  padding: int = 0 # Padding for non-periodic inputs
  output_scale: float = 1.0
  ...
  def __call__(self, x: jnp.ndarray) -> jnp.ndarray:
    ...
    # Lift the input to a higher dimension
    x1 = nn.Dense(self.width)(x)
    x2 = nn.Dense(self.width)(x)
    ...
    # Unpad
    if self.padding > 0:
      x1 = x1[:, :-self.padding, :-self.padding, :-self.padding, :]
      x2 = x2[:, :-self.padding, :-self.padding, :-self.padding, :]

    # Project to the output channels
    x1 = nn.Dense(self.channels_last_proj)(x1)
    x1 = self.activation(x1)
    x1 = nn.Dense(self.out_channels_1)(x1)

    x2 = nn.Dense(self.channels_last_proj)(x2)
    x2 = self.activation(x2)
    x2 = nn.Dense(self.out_channels_2)(x2)

    return self.output_scale * jnp.concatenate([x1, x2], axis=-1)
...
class FourierNeuralOperator3DFullSeparate(nn.Module):
  r'''
  Fourier Neural Operator for 2D signals.
  ...
  def __call__(self, x: jnp.ndarray) -> jnp.ndarray:
    ...
    x1 = nn.Dense(self.width)(x)
    x2 = nn.Dense(self.width)(x)
    x3 = nn.Dense(self.width)(x)
    x4 = nn.Dense(self.width)(x)
    ...
    if self.padding > 0:
      x1 = x1[:, :-self.padding, :-self.padding, :-self.padding, :]
      x2 = x2[:, :-self.padding, :-self.padding, :-self.padding, :]
      x3 = x3[:, :-self.padding, :-self.padding, :-self.padding, :]
      x4 = x4[:, :-self.padding, :-self.padding, :-self.padding, :]
    # Project to the output channels
    x1 = nn.Dense(self.channels_last_proj)(x1)
    x1 = self.activation(x1)
    x1 = nn.Dense(self.out_channels)(x1)

    x2 = nn.Dense(self.channels_last_proj)(x2)
    x2 = self.activation(x2)
    x2 = nn.Dense(self.out_channels)(x2)
    
    x3 = nn.Dense(self.channels_last_proj)(x3)
    x3 = self.activation(x3)
    x3 = nn.Dense(self.out_channels)(x3)

    x4 = nn.Dense(self.channels_last_proj)(x4)
    x4 = self.activation(x4)
    x4 = nn.Dense(self.out_channels)(x4)

    return self.output_scale * jnp.concatenate([x1, x2, x3, x4], axis=-1)
