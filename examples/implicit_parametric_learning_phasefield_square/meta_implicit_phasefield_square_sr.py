...
working_directory_name = 'meta_learning_phasefield_Neumann3'
case_dir = os.path.join('.', working_directory_name)
# create_clean_directory(working_directory_name)
sys.stdout = Logger(os.path.join(case_dir,working_directory_name+".log"))

# problem setup
model_settings = {"L":1,"N":151,
...
if create_random_coefficients:
    def generate_random_smooth_patterns(L, N, num_samples=1, smoothness_levels=[0.05, 0.1, 0.2, 0.3, 0.4]):
        """
    ...
    coeffs_matrix = generate_random_smooth_patterns(model_settings["L"],model_settings["N"]*mesh_res_rate)
else:
...
for test in range(eval_start_id,eval_end_id):
    ...
    plot_mesh_vec_data_phasefield(1,[coeffs_matrix[eval_id],FOL_T[:,time_list[0]],FOL_T[:,time_list[1]],FOL_T[:,time_list[2]]],
                ["","","",""],
                fig_title="",cmap = "jet",
                file_name=os.path.join(case_dir,f"SR_FOL-dist_test_{eval_id}.png"))
    
    plot_mesh_vec_data_phasefield(1,[coeffs_matrix[eval_id],FE_T[:,time_list[0]],FE_T[:,time_list[1]],FE_T[:,time_list[2]]],
                ["","","",""],
                fig_title="",cmap = "jet",
                file_name=os.path.join(case_dir,f"SR_FE-dist_test_{eval_id}.png"))
    plot_mesh_vec_data(1,[coeffs_matrix[eval_id],absolute_error[:,time_list[0]],absolute_error[:,time_list[1]],absolute_error[:,time_list[2]]],
                ["","","",""],
                fig_title="",cmap = "jet",
                file_name=os.path.join(case_dir,f"SR_FOL-FE-error-dist_test_{eval_id}.png"))
