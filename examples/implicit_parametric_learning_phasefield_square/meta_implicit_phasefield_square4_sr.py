...
import optax
import numpy as np
from fol.loss_functions.phasefield import PhaseFieldLoss2DQuad
...
working_directory_name = 'meta_learning_phasefield_Neumann3_copy'
...
Dirichlet_BCs = False
material_dict = {"rho":1.0,"cp":1.0,"dt":0.001,"epsilon":0.1}
phasefield_loss_2d = PhaseFieldLoss2DQuad("phasefield_loss_2d",loss_settings={"dirichlet_bc_dict":bc_dict,
                                                                            "num_gp":2,
                                                                            "material_dict":material_dict},
                                                                            fe_mesh=fe_mesh)
phasefield_loss_2d_pred = PhaseFieldLoss2DQuad("phasefield_loss_2d_pred",loss_settings={"dirichlet_bc_dict":bc_dict,
                                                                            "num_gp":2,
                                                                            "material_dict":material_dict},
...
if create_random_coefficients:
    def generate_random_smooth_patterns(L, N, num_samples=10, smoothness_levels=[0.05, 0.1, 0.2, 0.3, 0.4]):
        """
        ...
        for length_scale in smoothness_levels:
            ...
            scaled_y_samples = np.array([2*(y_sample - np.min(y_sample)) / (np.max(y_sample) - np.min(y_sample)) - 1
    ...
    coeffs_matrix = generate_random_smooth_patterns(model_settings["L"],model_settings["N"]*mesh_res_rate)[0].reshape(1,-1)
    # coeffs_matrix = np.full((1,fe_mesh.GetNumberOfNodes()),0.0)
else:
    coeffs_matrix = np.load("../training_data/gaussian_N51_num12000.npy")[0].reshape(1,-1)
...
for test in range(eval_start_id,eval_end_id):
    ...
    plot_mesh_vec_data_phasefield(1,[coeffs_matrix[eval_id],FOL_T[:,time_list[0]],FOL_T[:,time_list[1]],FOL_T[:,time_list[2]]],
                ["","","",""],
                fig_title="",cmap = "jet",
                file_name=os.path.join(case_dir,f"SR_FOL-dist_test_{eval_id}_zssr.png"))
    
    plot_mesh_vec_data_phasefield(1,[coeffs_matrix[eval_id],FE_T[:,time_list[0]],FE_T[:,time_list[1]],FE_T[:,time_list[2]]],
                ["","","",""],
                fig_title="",cmap = "jet",
                file_name=os.path.join(case_dir,f"SR_FE-dist_test_{eval_id}_zssr.png"))
    plot_mesh_vec_data(1,[coeffs_matrix[eval_id],absolute_error[:,time_list[0]],absolute_error[:,time_list[1]],absolute_error[:,time_list[2]]],
                ["","","",""],
                fig_title="",cmap = "jet",
                file_name=os.path.join(case_dir,f"SR_FOL-FE-error-dist_test_{eval_id}_zssr.png"))
