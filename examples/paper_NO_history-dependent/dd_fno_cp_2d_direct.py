...
working_directory_name = 'nn_output_fno_cp_2d_direct'
...
X_train, Y_train, scales = create_time_channel_set(sim_ids=train_sim_ids, increments=increments[:-extrapolate], which_data=which_data, 
                                           N=N, excluded_idx=excluded_idx,path=path, dtype=np.float32,normalize=True)
X_val, Y_val, _ = create_time_channel_set(sim_ids=val_sim_ids, increments=increments[:-extrapolate], which_data=which_data, 
                                           N=N, excluded_idx=excluded_idx,path=path, dtype=np.float32,normalize=True)
X_test, Y_test, _ = create_time_channel_set(sim_ids=test_sim_ids, increments=increments, which_data=which_data, 
                                           N=N, excluded_idx=excluded_idx,path=path, dtype=np.float32,normalize=True)
...
plot_to_check(X_train,N,window_lenght=1, channel_length=Cin, indices=[0,98,196], case_dir=case_dir,plot_name='train')
...
test_end_id = 10
batch_size = 8
dd_fno_pr_learning.Train(train_set=(X_train[train_start_id:,:],Y_train[train_start_id:,:]),
                        test_set=(X_val[val_start_id:,:],Y_val[val_start_id:,:]),
...
for case_id in [0, 120, 280, 420, 890, 1345, 1367, 1992]:   # less than train_/test_sim_ids
    ...
    gt_sol = gt_loader(which_data=which_data,sim_id=case_id,increments=increments,path=path,excluded_idx=excluded_idx,N=N)
    steps = len(increments)
    w = 1
    x0=gt_sol[0,:].reshape(N,N,Cout)
    x0_norm = x0 / np.max(x0,axis=(0,1))
    fno_sol = rollout_direct(model=dd_fno_pr_learning, x0=x0_norm, steps=(steps), N=N, C=Cout, w=w).squeeze().reshape(steps,-1)
    print(f"fno_sol shape: {fno_sol.shape}")
    print(f"gt_sol shape: {gt_sol.shape}")
    assert fno_sol.shape == (steps,N*N*Cout)
    assert gt_sol.shape == (steps,N*N*Cout)
    ...
    von_mises_plot(model_vm_pred,gt_vm,abs_err,(steps - w),N,case_id,case_dir,f'{output_name}_train_set',start_id=w)
    radial_power_spectrum_plot(model_vm_pred.reshape(fno_sol.shape[0],N,N),gt_vm.reshape(fno_sol.shape[0],N,N),
                               (steps - w),N,case_id,case_dir,f'{output_name}_RPS_train_set',start_id=w)
...
for case_id in [2258, 2298, 2302, 2345, 2379, 2421, 2462, 2492]:   # less than train_/test_sim_ids
    ...
    gt_sol = gt_loader(which_data=which_data,sim_id=case_id,increments=increments,path=path,excluded_idx=excluded_idx,N=N)  # (steps,N*N*C)
    steps = len(increments)
    w = 1
    x0=gt_sol[0,:].reshape(N,N,Cout)
    x0_norm = x0 / np.max(x0,axis=(0,1))
    fno_sol = rollout_direct(model=dd_fno_pr_learning, x0=x0_norm, steps=(steps), N=N, C=Cout, w=w).squeeze().reshape(steps,-1)  # (steps,N*N*C)
    print(f"fno_sol shape: {fno_sol.shape}")
    print(f"gt_sol shape: {gt_sol.shape}")
    assert fno_sol.shape == (steps,N*N*Cout)
    assert gt_sol.shape == (steps,N*N*Cout)
    ...
    output_name = list(type_index.keys())[list(type_index.values()).index(i)]
    von_mises_plot(model_vm_pred,gt_vm,abs_err,(steps - w),N,case_id,case_dir,f'{output_name}_test_set',start_id=w)
    radial_power_spectrum_plot(model_vm_pred.reshape(fno_sol.shape[0],N,N),gt_vm.reshape(fno_sol.shape[0],N,N),
                               (steps - w),N,case_id,case_dir,f'{output_name}_RPS_test_set',start_id=w)
    plot_value_1d(pred_field_list=[model_vm_pred[1,:],model_vm_pred[4,:],model_vm_pred[-1,:]],
                  gt_field_list=[gt_vm[1,:],gt_vm[4,:],gt_vm[-1,:]],
...
print(f"steps: {steps} , N: {N}, C: {Cout}, w: {w}")
#   shape: (steps,B,N*N*C)
excluded_idx = excluded_idx or []
valid_sim_ids = [s for s in range(*test_sim_ids) if s not in excluded_idx]
B = len(valid_sim_ids)
gt_sol_batch = np.zeros((steps,B,N*N*Cout))
fno_sol_batch = np.zeros((steps,B,N*N*Cout))
for ind,sim_id in enumerate(valid_sim_ids):    
    gt_sol = gt_loader(which_data=which_data,sim_id=sim_id,increments=increments,path=path,excluded_idx=excluded_idx,N=N)
    # x0=gt_sol[0,...]
    x0=gt_sol[0,...].reshape(N,N,Cout)
    norm_scales = scales["physical"]
    x0_norm = x0 / norm_scales
    fno_sol = rollout_direct(model=dd_fno_pr_learning, x0=x0_norm, steps=steps, N=N, C=Cout, w=w).squeeze()
    fno_sol_rescaled = (fno_sol * norm_scales)
    gt_sol_batch[:,ind,:] = gt_sol.reshape(steps,-1)
    fno_sol_batch[:,ind,:] = fno_sol_rescaled.reshape(steps,-1)
...
error_time_plot_(error_data_dict,which_channel=0,w=w,filename='boxplot_error_in_time.png',case_dir=case_dir)
