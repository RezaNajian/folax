...
def safe_rename(src: str, dst: str):
    """Rename src->dst if src exists. Overwrite dst if it exists."""
...
def _parse_ids(ids_str: str, n: int):
    ...
    if not out:
        raise ValueError("train_ids parsed to empty list.")
    return out
...
def _clean_mkdir(path: str):
    """Absolute-path-safe clean create."""
...
def _read_training_residual_csv(csv_path: str):
    """
    Expected columns (from your tracker):
      epoch, total_loss, residual_rms_batch_mean
    """
...
def _summarize_training(csv_path: str, residual_target: float):
    out = {
        ...
        "last_epoch": np.nan,                 # <-- NEW (index, e.g., 199)
    ...
    out["last_epoch"] = int(epoch[-1])      # <-- NEW
...
if __name__ == "__main__":
    ...
    out_root = str(script_dir / "PHASE_2_OTF_Fourier")
