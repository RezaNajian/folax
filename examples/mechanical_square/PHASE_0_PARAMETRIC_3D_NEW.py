...
def _parse_ids(ids_str: str, n: int):
    """
    Same semantics as your 2D script:
...
def _save_2x2_panel(K_vec, fno_ux, fe_ux, abs_err_ux, Nx, Ny, Nz, out_path):
    ...
    im0 = axs[0].imshow(Ksl, origin="lower"); axs[0].set_title("Heterogeneity (mid-z)"); fig.colorbar(im0, ax=axs[0], fraction=0.046)
    im1 = axs[1].imshow(Fsl, origin="lower"); axs[1].set_title("FNO_Ux (mid-z)");        fig.colorbar(im1, ax=axs[1], fraction=0.046)
    im2 = axs[2].imshow(Esl, origin="lower"); axs[2].set_title("FE_Ux (mid-z)");         fig.colorbar(im2, ax=axs[2], fraction=0.046)
    im3 = axs[3].imshow(Asl, origin="lower"); axs[3].set_title("absolute_error_Ux");     fig.colorbar(im3, ax=axs[3], fraction=0.046)
...
def _copy_training_artifacts(train_case_dir: str, case_dir: str, train_tag: str):
    # history image
    ...
    for pat in patterns:
        for src in sorted(glob.glob(pat)):
            ...
            dst = os.path.join(case_dir, base)
            if base in copied:
                continue
            try:
...
if __name__ == "__main__":
    ...
    out_root = str(script_dir / "PHASE_0_PARAMETRIC_3D_Fourier")
