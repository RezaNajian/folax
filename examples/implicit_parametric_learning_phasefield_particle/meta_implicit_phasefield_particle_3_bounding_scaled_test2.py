...
working_directory_name = 'meta_learning_phasefield_particle_3_bounding_scaled'
...
fe_mesh_pred = Mesh("fol_io","Li_battery_particle_scaled.med",'../meshes/')
...
if create_random_coefficients:
    def generate_random_smooth_patterns_from_mesh(coords, num_samples=10,smoothness_levels=[0.15, 0.2, 0.3, 0.4, 0.5]):
        """
    ...
    np.save(os.path.join(case_dir,f"particle_pf_2d_gaussian_N{fe_mesh.GetNodesCoordinates().shape[0]}_num{coeffs_matrix.shape[0]}.npy"), coeffs_matrix)
else:
    coeffs_matrix = np.load("../training_data/particle_pf_2d_gaussian_N51_num10000.npy")
...
for test in range(eval_start_id,eval_end_id):
    ...
    time_list2 = [0,1,4,9]
    FEPlotter.plot_triangulated(fe_mesh.GetNodesCoordinates(),fe_mesh.GetElementsNodes("triangle"),
                                [coeffs_matrix[eval_id],FOL_T[:,time_list[0]],FOL_T[:,time_list[1]],FOL_T[:,-1]],
                                filename=os.path.join(case_dir,"FOL_predictions_all2.png"),value_range=(-1,1))
    FEPlotter.plot_triangulated(fe_mesh.GetNodesCoordinates(),fe_mesh.GetElementsNodes("triangle"),
                                [coeffs_matrix[eval_id],FE_T[:,time_list[0]],FE_T[:,time_list[1]],FE_T[:,-1]],
                                filename=os.path.join(case_dir,"FE_solutions_all2.png"),value_range=(-1,1))
    FEPlotter.plot_triangulated_error(fe_mesh.GetNodesCoordinates(),fe_mesh.GetElementsNodes("triangle"),
                                ...
                                filename=os.path.join(case_dir,"FOL-FE_errors_all2.png"))
    FEPlotter.plot_triangulated(fe_mesh.GetNodesCoordinates(),fe_mesh.GetElementsNodes("triangle"),
                                [FOL_T[:,time_list2[0]],FOL_T[:,time_list2[1]],FOL_T[:,time_list2[2]],FOL_T[:,time_list2[3]]],
                                filename=os.path.join(case_dir,"FOL_predictions_initialsteps2.png"),value_range=(-1,1))
    FEPlotter.plot_triangulated(fe_mesh.GetNodesCoordinates(),fe_mesh.GetElementsNodes("triangle"),
                                [FE_T[:,time_list2[0]],FE_T[:,time_list2[1]],FE_T[:,time_list2[2]],FE_T[:,time_list2[3]]],
                                filename=os.path.join(case_dir,"FE_solutions_initialsteps2.png"),value_range=(-1,1))
    FEPlotter.plot_triangulated_error(fe_mesh.GetNodesCoordinates(),fe_mesh.GetElementsNodes("triangle"),
                                [absolute_error[:,time_list2[0]],absolute_error[:,time_list2[1]],absolute_error[:,time_list2[2]],absolute_error[:,time_list2[3]]],
                                filename=os.path.join(case_dir,"FOL-FE_errors_initialsteps2.png"))
