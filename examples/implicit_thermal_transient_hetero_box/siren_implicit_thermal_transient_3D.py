...
mesh_res_list = [16,32,64]

for k, mesh_res in enumerate(mesh_res_list):
    # directory & save handling
    working_directory_name = 'siren_implicit_thermal_3D'+'_'+str(mesh_res)
    case_dir = os.path.join('.', working_directory_name)
    create_clean_directory(working_directory_name)
    sys.stdout = Logger(os.path.join(case_dir,working_directory_name+".log"))

    # problem setup
    model_settings = {"L":1,"N":mesh_res,
    ...
    fe_line = np.linspace(0,1,mesh_res)

    import matplotlib.pyplot as plt
    plt.plot(fe_line, FE_T_reshaped.flatten())

plt.legend(["16*16*16", "32*32*32","64*64*64"])
