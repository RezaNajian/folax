...
freq_sets = [
    ...
    (np.array([1, 2, 4, 8]), np.array([1, 2, 4, 8]), np.array([1, 2, 4, 8])),
    (np.array([2, 4, 6]),    np.array([2, 4, 6]), np.array([2, 4, 6])),    
    (np.array([1, 3, 5, 7]), np.array([1, 3, 5, 7]), np.array([1, 3, 5, 7])), 
]
...
E_matrix = K_matrix
np.save(os.path.join(case_dir, 'fourier_samples.npy'), K_matrix)
...
FE_TUV = np.array(nonlinear_fe_solver.SolveMulti(K_matrix[test_start_id:test_end_id,:].flatten(),np.zeros((fe_mesh.GetNumberOfNodes()*output_size))))  #
