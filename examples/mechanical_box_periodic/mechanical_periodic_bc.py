...
from fol.loss_functions.fe_kinematics import b_matrix_2d, b_matrix_3d, d_matrix_2d, d_matrix_3d, n_matrix_2d, n_matrix_3d
import jax
...
class MechanicalLoss(FiniteElementLoss):
    ...
    def Initialize(self) -> None:
        ...
        if self.dim == 2:
            ...
            self.macro_deformation_grad = jnp.array(self.loss_settings["macro_deformation_grad"]) if "macro_deformation_grad" in self.loss_settings else jnp.eye(2)
            self.identity_matrix = jnp.eye(2)
        else:
            self.dim = 3
            ...
            if "body_foce" in self.loss_settings:
                self.body_force = jnp.array(self.loss_settings["body_foce"])
            self.identity_matrix = jnp.eye(3)
            self.macro_deformation_grad = jnp.array(self.loss_settings["macro_deformation_grad"]) if "macro_deformation_grad" in self.loss_settings else jnp.eye(3)
    ...
    def ComputeElement(self,xyze,de,uvwe):
        ...
        # F_vec = de[0,1:5]
        F_vec = de[0,1:]
        macro_deformation_grad = F_vec.reshape(self.dim,self.dim) 
        def compute_at_gauss_point(gp_point,gp_weight):
            ...
            e_at_gauss = jnp.dot(N_vec, e_element)
            ...
            # eps_macro = self.macro_deformation_grad @ xyze.flatten()
            # gp_f_macro = gp_weight * detJ * e_at_gauss * (B_mat.T @ (self.D @ eps_macro))
        ...
        # disp_macro = xyze[:,:2]@ (self.macro_deformation_grad - self.identity_matrix).T
        disp_macro = xyze[:,:self.dim]@ (macro_deformation_grad - self.identity_matrix).T
        disp = uvwe + disp_macro.reshape(-1,1)
        # disp = uvwe
        element_residuals = jax.lax.stop_gradient(Se @ disp  - Fe)
        return  ((disp.T @ element_residuals)[0,0]), (Se @ disp - Fe), Se
