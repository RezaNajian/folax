...
class FiniteElementLoss(Loss):
    ...
    def Initialize(self,reinitialize=False) -> None:
        ...
        self.nse = int(self.fe_mesh.GetNumberOfElements(self.element_type)*2)
        # fe element
        ...
        self.ConstructPMat()
        self.ConstructInputPMat()
        self.periodic_bcs = self.loss_settings.get("periodic_bcs", False)
    ...
    def GetPeriodicDofVector(self,known_dofs: jnp.array,unknown_dofs: jnp.array) -> jnp.array:
        """
        Construct a full DOF vector including periodic boundary values.
        ...
        Returns:
            jax.numpy.ndarray:
                Full DOF vector with periodic boundary entries applied.
        """
        return self.full_dof_vector_function(known_dofs,unknown_dofs)
    ...
    def ComputeBatchLossMulti(self,batch_params:jnp.array,batch_dofs:jnp.array):
        """
        Compute the finite element loss over a batch of parameter samples and
        their associated solution fields.
        ...
        batch_params = jnp.atleast_2d(batch_params)
        batch_params = batch_params.reshape(batch_params.shape[0], -1, batch_params.shape[-1])
        batch_dofs = jnp.atleast_2d(batch_dofs)
        batch_dofs = batch_dofs.reshape(batch_dofs.shape[0], -1)
        BC_applied_batch_dofs = self.GetFullDofVector(batch_params,batch_dofs)
    ...
    def ConstructPMat(self):
        """
        Construct the periodic projection matrix P such that

            u_full = P @ u_reduced

        where slave DOFs share the same reduced DOF as their master DOFs.
        """
        if "periodic_bc_dict" not in self.loss_settings:
            ndofs = self.total_number_of_dofs
            indices = jnp.stack([jnp.arange(ndofs), jnp.arange(ndofs)], axis=1)
            data = jnp.ones((ndofs,), dtype=jnp.float32)
            P_mat = sparse.BCOO((data, indices), shape=(ndofs, ndofs))
            self.P_mat = P_mat
            self.PT_mat = P_mat.transpose()
            self.full_to_reduced = jnp.arange(ndofs, dtype=jnp.int32)
            self.reduced_number_of_dofs = ndofs
            return P_mat

        periodic_bc_dict = self.loss_settings["periodic_bc_dict"]
        ndofs = self.total_number_of_dofs

        # -------------------------------------------------
        # 1. full DOF -> representative full DOF (master) map
        # -------------------------------------------------
        dof_map = jnp.arange(ndofs, dtype=jnp.int32)


        for _, (master_nodes, slave_nodes) in periodic_bc_dict.items():
            master_nodes = jnp.asarray(master_nodes, dtype=jnp.int32)
            slave_nodes  = jnp.asarray(slave_nodes, dtype=jnp.int32)

            for dof_id in range(self.number_dofs_per_node):
                master_dofs = master_nodes * self.number_dofs_per_node + dof_id
                slave_dofs  = slave_nodes  * self.number_dofs_per_node + dof_id
                dof_map = dof_map.at[slave_dofs].set(master_dofs)

        # optional: transitive closure for corner nodes / chained mappings
        # dof_map = dof_map[dof_map]

        # -------------------------------------------------
        # 2. compress representative full DOF ids to reduced ids
        # -------------------------------------------------
        unique_reps = jnp.unique(dof_map)
        reduced_ndofs = unique_reps.shape[0]

        # full representative id -> reduced id
        rep_to_reduced = -jnp.ones((ndofs,), dtype=jnp.int32)
        rep_to_reduced = rep_to_reduced.at[unique_reps].set(jnp.arange(reduced_ndofs, dtype=jnp.int32))

        full_to_reduced = rep_to_reduced[dof_map]

        # -------------------------------------------------
        # 3. build P in BCOO format
        #    row = full dof id
        #    col = reduced dof id
        #    value = 1
        # -------------------------------------------------
        rows = jnp.arange(ndofs, dtype=jnp.int32)
        cols = full_to_reduced
        indices = jnp.stack([rows, cols], axis=1)
        data = jnp.ones((ndofs,), dtype=jnp.float32)

        P_mat = sparse.BCOO((data, indices), shape=(ndofs, reduced_ndofs))
        # P_mat = P_mat.sum_duplicates()

        self.P_mat = P_mat
        self.PT_mat = P_mat.transpose()
        self.full_to_reduced = full_to_reduced
        self.reduced_number_of_dofs = reduced_ndofs
        self.representative_dofs = unique_reps

    def ConstructInputPMat(self):
        """
        Construct the periodic projection matrix P such that
        ...
        """
        if "periodic_bc_dict" not in self.loss_settings:
            ndofs = self.fe_mesh.GetNumberOfNodes()
            indices = jnp.stack([jnp.arange(ndofs), jnp.arange(ndofs)], axis=1)
            data = jnp.ones((ndofs,), dtype=jnp.float32)
            P_mat = sparse.BCOO((data, indices), shape=(ndofs, ndofs))
            self.P_mat = P_mat
        ...
        for _, (master_nodes, slave_nodes) in periodic_bc_dict.items():
            ...
            for dof_id in range(1):
                master_dofs = master_nodes  + dof_id
                slave_dofs  = slave_nodes + dof_id
                dof_map = dof_map.at[slave_dofs].set(master_dofs)
        ...
        self.input_P_mat = P_mat
        self.input_PT_mat = P_mat.transpose()
        self.input_full_to_reduced = full_to_reduced
        self.input_reduced_number_of_dofs = reduced_ndofs
        self.input_representative_dofs = unique_reps

    def GetReducedDofVector(self, full_dof_vector: jnp.array) -> jnp.array:
        """
        Map a full DOF vector to the reduced DOF space using the projection matrix P.

        Args:
            full_dof_vector (jax.numpy.ndarray):
                Global DOF vector in the full space.

        Returns:
            jax.numpy.ndarray:
                Reduced DOF vector after applying the projection.
        """
        return full_dof_vector[self.representative_dofs]
    
    def GetReducedDofVectorBatch(self, full_dof_vector: jnp.array) -> jnp.array:
        """
        Map a batch of full DOF vectors to the reduced DOF space using the projection matrix P.

        Args:
            full_dof_vector (jax.numpy.ndarray):
                Batch of global DOF vectors in the full space.

        Returns:
            jax.numpy.ndarray:
                Reduced DOF vector after applying the projection.
        """
        return full_dof_vector[:, self.input_representative_dofs]
    
    def ReduceJacobianAndResidual(self, jacobian: sparse.BCOO, residual: jnp.array) -> tuple[sparse.BCOO, jnp.array]:
        """
        Reduce the global Jacobian and residual to the representative DOF space.

        This method applies the projection matrix P to both the Jacobian and
        residual to obtain their reduced forms corresponding to the representative
        DOFs (masters).

        Args:
            jacobian (jax.experimental.sparse.BCOO):
                Global Jacobian matrix in BCOO format.
            residual (jax.numpy.ndarray):
                Global residual vector.

        Returns:
            Tuple[jax.experimental.sparse.BCOO, jax.numpy.ndarray]:
                - Reduced Jacobian matrix in BCOO format.
                - Reduced residual vector.
        """
        # reduced_jacobian = self.PT_mat @ jacobian @ self.P_mat
        rows = jacobian.indices[:, 0]
        cols = jacobian.indices[:, 1]
        data = jacobian.data

        new_rows = self.full_to_reduced[rows]
        new_cols = self.full_to_reduced[cols]

        new_indices = jnp.stack([new_rows, new_cols], axis=1)

        reduced_jacobian = sparse.BCOO(
            (data, new_indices),
            shape=(self.reduced_number_of_dofs, self.reduced_number_of_dofs)
        )

        reduced_jacobian = reduced_jacobian.sum_duplicates()
        reduced_residual = self.PT_mat @ residual
        return reduced_jacobian, reduced_residual

    def ExpandReducedDeltaDofVector(self, reduced_delta_dof_vector: jnp.array) -> jnp.array:
        """
        Expand a reduced delta DOF vector back to the full DOF space.

        This method maps a delta DOF vector defined in the reduced representative
        DOF space back to the full DOF space using the projection matrix P.

        Args:
            reduced_delta_dof_vector (jax.numpy.ndarray):
                Delta DOF vector in the reduced representative DOF space.

        Returns:
            jax.numpy.ndarray:
                Expanded delta DOF vector in the full DOF space.
        """
        return self.P_mat @ reduced_delta_dof_vector
    
    def ExpandReducedDeltaDofVectorBatch(self, reduced_delta_dof_vector: jnp.array) -> jnp.array:
        """
        Expand a reduced delta DOF vector back to the full DOF space.
        ...
        Returns:
            jax.numpy.ndarray:
                Expanded delta DOF vector in the full DOF space.
        """
        return (self.P_mat @ reduced_delta_dof_vector.T).T
