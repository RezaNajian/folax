...
def main(solve_FE=False,ifol_num_epochs=10,clean_dir=False):
    ...
    working_directory_name = "sample_gen_2D_fourier_voronoi_new"
    case_dir = os.path.join('.', working_directory_name)
    fol_info(f"You are about to overwrite or create a file in {case_dir}")
    ...
    if confirm == "y":
        ...
        raise ValueError('insert "y" or "n"!')
    ...
    if confirm == "y":
        with open (file_path, 'wb') as f:
            i = 1
            num_samples = 5
            for x_freqs in freq_list:
                for y_freqs in freq_list:
                    mixed_fourier_sample_dict[f"series_{i}"] = {}
                    fourier_control_settings_copy = fourier_control_settings.copy()
                    fourier_control_settings_copy.update({"x_freqs":x_freqs, "y_freqs":y_freqs})
                    mixed_fourier_sample_dict[f"series_{i}"]["settings"] = fourier_control_settings_copy
                    fourier_control = FourierControl("fourier_control",fourier_control_settings_copy,fe_mesh)
                    fourier_control.Initialize()
                    coeffs_matrix, K_matrix = create_random_fourier_samples(fourier_control=fourier_control,
                                                                                  numberof_sample=num_samples,random_seed_key=52)
                    mixed_fourier_sample_dict[f"series_{i}"]["K_matrix"] = K_matrix
                    mixed_fourier_sample_dict[f"series_{i}"]["coeffs_matrix"] = coeffs_matrix
                    i += 1
            pickle.dump(mixed_fourier_sample_dict,f)
            fol_info(f"{file_path} saved successfully!")
    elif confirm == "n":
        with open(file_path, 'rb') as f:
            mixed_fourier_sample_dict = pickle.load(f)
        fol_info(f"The file {file_path} loaded!")
    else:
        fol_info("Invalid input. Please enter Y or N.")
    

    # dual voronoi samples
    ...
    seed_list = [16,20,25,32,50,64,75,100,128]
    voronoi_sample_dict = {}
    file_path = os.path.join(case_dir,f"ifol_voronoi_dual_test_samples_res_{model_settings['N']}.pkl")
    ...
    if confirm == "y":
        with open (file_path, 'wb') as f:
            ...
            for num_seeds in seed_list:
                voronoi_sample_dict[f"series_{i}"] = {}
                voronoi_control_settings_copy = voronoi_control_settings.copy()
                voronoi_control_settings_copy.update({"number_of_seeds":num_seeds})
                voronoi_sample_dict[f"series_{i}"]["settings"] = voronoi_control_settings_copy
                voronoi_control = VoronoiControl2D("first_voronoi_control",voronoi_control_settings_copy,fe_mesh)
                voronoi_control.Initialize()
                coeffs_matrix, K_matrix = create_random_voronoi_samples(voronoi_control=voronoi_control,
                                                                            number_of_sample=num_samples,dim=2,random_seed_key=52)
                voronoi_sample_dict[f"series_{i}"]["K_matrix"] = K_matrix
                voronoi_sample_dict[f"series_{i}"]["coeffs_matrix"] = coeffs_matrix
                i += 1
            pickle.dump(voronoi_sample_dict,f)
            fol_info(f"{file_path} saved successfully!")
    elif confirm == "n":
        with open(file_path, 'rb') as f:
            voronoi_sample_dict = pickle.load(f)
        fol_info(f"The file {file_path} loaded!")
    else:
        fol_info("Invalid input. Please enter Y or N.")
    

    # multi voronoi samples
    ...
    if confirm == "y":
        with open (file_path, 'wb') as f:
            ...
            num_samples = 5
            for num_seeds in seed_list:
                voronoi_sample_multi_dict[f"series_{i}"] = {}
                voronoi_control_settings_copy = voronoi_multi_control_settings.copy()
                voronoi_control_settings_copy.update({"number_of_seeds":num_seeds})
                voronoi_sample_multi_dict[f"series_{i}"]["settings"] = voronoi_control_settings_copy
                voronoi_multi_control = VoronoiControl2D("first_voronoi_control",voronoi_control_settings_copy,fe_mesh)
                voronoi_multi_control.Initialize()
                coeffs_matrix, K_matrix = create_random_voronoi_samples(voronoi_control=voronoi_multi_control,
                                                                            number_of_sample=num_samples,dim=2,random_seed_key=52)
                voronoi_sample_multi_dict[f"series_{i}"]["K_matrix"] = K_matrix
                voronoi_sample_multi_dict[f"series_{i}"]["coeffs_matrix"] = coeffs_matrix
                i += 1
            pickle.dump(voronoi_sample_multi_dict,f)
            fol_info(f"{file_path} saved successfully!")
    elif confirm == "n":
        with open(file_path, 'rb') as f:
            voronoi_sample_multi_dict = pickle.load(f)
        fol_info(f"The file {file_path} loaded!")
    else:
        fol_info("Invalid input. Please enter Y or N.")
    ...
    plot_random_K_matrix(sample_dict=mixed_fourier_sample_dict,file_name=os.path.join(case_dir,"plot_fourier"))
    plot_random_K_matrix(sample_dict=voronoi_sample_dict,file_name=os.path.join(case_dir,"plot_voronoi"))
    plot_random_K_matrix(sample_dict=voronoi_sample_multi_dict,file_name=os.path.join(case_dir,"plot_voronoi_multi"))


    fourier_coeffs_matrix, fourier_K_matrix = fourier_to_K_matrix(mixed_fourier_sample_dict)
    ...
    voronoi_K_matrix = voronoi_to_K_matrix(voronoi_sample_dict)
    ...
    voronoi_multi_K_matrix = voronoi_to_K_matrix(voronoi_sample_multi_dict)
