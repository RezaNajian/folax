...
def main(ifol_num_epochs=10,solve_FE=False,solve_FE_hybrid=False,clean_dir=False):
    ...
    working_directory_name = "hybrid_mechanical_2d_bc_0.5"
    ...
    fol_info(f"K_matrix shape: {K_matrix.shape}")
    ...
    for eval_id in [otf_id]:
        ...
        fe_mesh[f'U_iFOL_{eval_id}'] = ifol_uvw.reshape((fe_mesh.GetNumberOfNodes(), 2))
        ...
        if solve_FE:
            ...
            fe_mesh[f'U_FE_{eval_id}'] = FE_UVW.reshape((fe_mesh.GetNumberOfNodes(), 2))
            abs_err = abs(FE_UVW.reshape(-1,1) - ifol_uvw.reshape(-1,1))
            fe_mesh[f"absolute_error_{eval_id}"] = abs_err.reshape((fe_mesh.GetNumberOfNodes(), 2))
            fe_mesh[f"FE_stress_{eval_id}"] = fe_stress.reshape((fe_mesh.GetNumberOfNodes(), 3))
            fe_mesh[f"absolute_stress_error_{eval_id}"] = abs_stress_err.reshape((fe_mesh.GetNumberOfNodes(), 3))



            # save U displacement field in pkl
            ...
            with open(f"U_base_resolution_bc_{model_settings['Ux_right']}.pkl" , 'wb') as f:
            ...
            nonlin_fe_solver = FiniteElementNonLinearResidualBasedSolver("nonlin_fe_solver",mechanical_loss_2d,fe_setting)
            nonlin_fe_solver.Initialize()
            FE_H_UVW = np.array(nonlin_fe_solver.Solve(K_matrix[eval_id,:],ifol_uvw.reshape(2*fe_mesh.GetNumberOfNodes())))  
            fe_mesh[f'U_FE_hybrid_{eval_id}'] = FE_H_UVW.reshape((fe_mesh.GetNumberOfNodes(), 2))
            abs_err = abs(FE_H_UVW.reshape(-1,1) - ifol_uvw.reshape(-1,1))
            fe_mesh[f"absolute_error_hybrid_{eval_id}"] = abs_err.reshape((fe_mesh.GetNumberOfNodes(), 2))
            ...
            plot_settings = {"topology_field":K_matrix[eval_id,:], "ifol_sol_field":ifol_uvw, "fe_sol_field":FE_UVW, 
                             ...
                             "file_name":f"plot_ifol_fe_{eval_id}"}
            ...
            dump_output(ifol_model_settings=ifol_settings_dict,
                ...
                case_dir=case_dir,file_format='json')
