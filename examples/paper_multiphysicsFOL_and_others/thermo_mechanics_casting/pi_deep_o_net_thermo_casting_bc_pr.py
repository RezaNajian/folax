...
working_directory_name = 'pi_deep_o_net_thermo_mechanical'
...
branch_nn = MLP(name="branch_nn",
                input_size=1,
                hidden_layers=[characteristic_length*4]*2,
...
# create fol
deeponet_learning = PhysicsInformedDeepONetParametricOperatorLearningDBC(name="deeponet_learning",
                                                                        control=displ_control,
...
train_start_id = 0
train_end_id = 50
wanted_bc = np.array([[0.5]])  # T, Ux, Uy, Uz
bc_matrix = np.vstack((bc_matrix,wanted_bc))
print(bc_matrix.shape)

deeponet_learning.Train(train_set=(bc_matrix[train_start_id:train_end_id,:],),
          batch_size=1,
...
# num_steps = 1
FOL_TUVW = np.array(deeponet_learning.Predict((bc_matrix[-1,:].reshape(-1,1))))  
FOL_TUVW = FOL_TUVW.reshape((fe_mesh.GetNumberOfNodes(), num_dofs))
fe_mesh['sol_FOL'] = FOL_TUVW

# solve FE here
# Initialize the fe loss function with the desired boundary conditions
bc_dict = {"T":{"Inlet_top":wanted_bc[0],"F_bottom":0.001,"Surroundings":0.001},#
           "Ux":{"Inlet_top":0.0,"Inlet_bottom":0.0,"F_bottom":0.0},#"F_right1":model_settings["Ux_right"],"F_right1":model_settings["Ux_right"]
           "Uy":{"Inlet_top":0.0,"Inlet_bottom":0.0,"F_bottom":0.0},
           "Uz":{"Inlet_top":0.0,"F_bottom":0.0}}#,"F_rear":model_settings["Uz_back"]
...
FE_TUVW = np.array(nonlinear_fe_solver.Solve(test_K_coeff.flatten(),np.zeros((fe_mesh.GetNumberOfNodes()*num_dofs))))  #
