...
working_directory_name = 'iFOL_thermo_mechanical'
...
fol.Train(train_set=(bc_matrix[train_start_id:train_end_id,:],),
          batch_size=1,
...
# num_steps = 1
FOL_TUVW = np.array(fol.Predict((bc_matrix[-1,:].reshape(-1,1))))  
FOL_TUVW = FOL_TUVW.reshape((fe_mesh.GetNumberOfNodes(), output_size))
...
FE_TUVW = np.array(nonlinear_fe_solver.Solve(test_K_coeff.flatten(),np.zeros((fe_mesh.GetNumberOfNodes()*output_size))))  #
