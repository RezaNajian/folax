...
neuron_list = [64,64,64,64,64,64,128,256]
layer_list =  [ 1, 2, 3, 4, 5, 6,  2,  2]
plot_label = []
for i in range(len(neuron_list)):
    plot_label.append('Omega_'+str(neuron_list[i])+' Alpha_'+str(layer_list[i]))
num_steps = 100
relative_l2_error_list = np.zeros((len(neuron_list),num_steps+1))

for l, (omega, alpha) in enumerate(zip(neuron_list,layer_list)):

    # directory & save handling
    working_directory_name = 'siren_implicit_AllenCahn_2D_network_layer3'+'_'+str(omega)+'_'+str(alpha)
...
plot_relative_l2_error_multiple(time_plot,relative_l2_error_list,label_name=plot_label,file_name="iFOL_FEM_relative_L2_error_layer_study_all3.png")
np.savetxt("relative_l2_error_layer_study3.txt",np.hstack((time_plot.reshape(-1,1),relative_l2_error_list.T)))