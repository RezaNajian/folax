...
# num_steps = 1
FOL_TUV = np.array(fol.Predict((test_K_coeff,test_E_coeff)))  
...
FOLstress_at_nodes = GetStressVector2D(thermomech_loss_2d,fe_mesh, test_E_coeff.flatten(),
                                       FOL_UV.flatten(),FOL_T.flatten(),initial_temp.flatten())
FE_stress_at_nodes = GetStressVector2D(thermomech_loss_2d,fe_mesh, test_E_coeff.flatten(),
...
FOL_heat_flux_at_nodes = GetHeatFluxVector2D(thermomech_loss_2d,fe_mesh,test_K_coeff.flatten(),FOL_T.flatten())
FE_heat_flux_at_nodes = GetHeatFluxVector2D(thermomech_loss_2d,fe_mesh,test_K_coeff.flatten(),FE_T.flatten())
