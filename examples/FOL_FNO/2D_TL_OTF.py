...
def main(
    ...
    print(f"[PHASE_3_TL_OTF] skip_training={skip_training}")
    print(f"[PHASE_3_TL_OTF] use_fno_warmstart={use_fno_warmstart}")
    ...
    for sample_id in run_list:
        ...
        print("[PHASE_3_TL_OTF] Restored parametric checkpoint.")

        if skip_training:
            print("[PHASE_3_TL_OTF] skip_training=True -> using restored parametric model directly.")
            prediction_source = "parametric_only"
            train_time_s = np.nan
            train_stats = {
            ...
            prediction_source = "tl_otf"
            print("[PHASE_3_TL_OTF] Starting TL-OTF fine-tuning...")
            ...
            t_train0 = time.time()
            pi_fno_tl_otf.Train(
                train_set=(jnp.asarray(train_set),),
                test_set=(jnp.asarray(test_set),),
                test_frequency=100,
                batch_size=1,
...
if __name__ == "__main__":
    ...
    # NOTE: the 2D parametric results land in 2D_fourier_results/ at the workspace root,
    #       NOT next to this script. script_dir.parent.parent reaches the workspace root.
    param_case_dir = str(script_dir / "PHASE_0_PARAMETRIC_2D_Fourier_strain_25/PHASE_0_PARAMETRIC_train_fourier_control")
    out_root = str(script_dir / "PHASE_3_TL_OTF_Fourier_strain_25")
    ...
    for arg in args:
        if arg.startswith("fol_num_epochs="):
            ...
            print(
                ...
                "  out_root=/path/to/PHASE_3_TL_OTF_Fourier_*/\n"
            )
            sys.exit(1)
