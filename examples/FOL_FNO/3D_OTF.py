#!/usr/bin/env python3
# ============================================================
# PHASE_2_OTF_3D_NEW.py
#
# 3D OTF version created by mapping the working 2D OTF logic
# onto the working 3D parametric backbone.
#
# Main behavior:
#   - sets up the 3D Neo-Hookean hexa problem once
#   - loads (or optionally generates) the 3D Fourier PKL once
#   - parses train_ids / test_ids for bookkeeping and sample selection
#   - runs OTF training independently for each selected test sample
#   - restores the best per-sample checkpoint
...
def main(
    ...
    out_root: str = "PHASE_2_OTF_3D_Fourier",
):
    ...
    if (not create_new_samples) and (not os.path.exists(pkl_path)):
        raise FileNotFoundError(
            f"[PHASE_2_OTF_3D] pkl not found: {pkl_path}\n"
    ...
    if create_new_samples:
        ...
        print(f"[PHASE_2_OTF_3D] Wrote NEW pkl: {pkl_path}")
        print(f"[PHASE_2_OTF_3D] coeffs_matrix shape={coeffs_matrix.shape}, K_matrix shape={K_matrix.shape}")
    ...
    if "K_matrix" in d:
        ...
        print(f"[PHASE_2_OTF_3D] Found K_matrix in pkl: {K_matrix.shape}")
        ...
        print(f"[PHASE_2_OTF_3D] Found coeffs_matrix in pkl: {coeffs_matrix.shape}")
        ...
        print(f"[PHASE_2_OTF_3D] Computed K_matrix from coeffs_matrix: {K_matrix.shape}")

    else:
        raise KeyError("[PHASE_2_OTF_3D] pkl must contain either 'K_matrix' or 'coeffs_matrix'")

    if K_matrix.ndim != 2:
        raise ValueError(f"[PHASE_2_OTF_3D] K_matrix must be 2D, got {K_matrix.shape}")

    num_samples_eff, k_nodes = K_matrix.shape
    if k_nodes != n_nodes:
        raise ValueError(
            f"[PHASE_2_OTF_3D] K_matrix has {k_nodes} nodes, expected {n_nodes}. "
    ...
    print(f"[PHASE_2_OTF_3D] out_root={out_root}")
    print(f"[PHASE_2_OTF_3D] train_ids={train_ids} -> {len(train_list)} samples")
    print(f"[PHASE_2_OTF_3D] test_ids={test_ids}  -> {len(test_list)} samples")
    print(
        f"[PHASE_2_OTF_3D] Will run OTF for {len(eval_list)} test samples: "
        f"{eval_list[:10]}{'...' if len(eval_list) > 10 else ''}"
    )

    # ============================================================
    # 4) loop over selected test ids (one OTF model per sample)
    # ============================================================
    for sample_id in eval_list:
        label = _sanitize_label(labels_vec[sample_id]) if labels_vec is not None else default_label

        case_dir = os.path.join(out_root, f"PHASE_2_OTF_sample_{sample_id}_{label}")
        _clean_mkdir(case_dir)
        sys.stdout = Logger(os.path.join(case_dir, f"PHASE_2_OTF_sample_{sample_id}_{label}.log"))

        otf_tag = f"otf_identity_{label}_sid{sample_id}"
        otf_res_tracker = TrainingResidualTracker(out_dir=case_dir, tag=otf_tag)

        print(f"[PHASE_2_OTF_3D] sample_id={sample_id}, label={label}")
        print(f"[PHASE_2_OTF_3D] pkl_path={pkl_path}")
        print(f"[PHASE_2_OTF_3D] output_dir={case_dir}")
        ...
        # ============================================================
        # 5) build fresh FNO model for this sample
        # ============================================================
        fno_model = bridge.ToNNX(
            ...
            rngs=nnx.Rngs(0),
        ).lazy_init(K_vec.reshape(1, Nx, Ny, Nz, 1))

        fno_model = _patch_rng_keys(fno_model)

        params = nnx.state(fno_model, nnx.Param)
        total_params = sum(np.prod(x.shape) for x in jax.tree_util.tree_leaves(params))
        print(f"[PHASE_2_OTF_3D] total number of FNO network parameters: {total_params}")
        ...
        pi_fno_otf = PhysicsInformedFourierParametricOperatorLearning3D(
            name="pi_fno_otf_identity_3d",
        ...
        # ============================================================
        train_set = jnp.asarray(K_vec.reshape(1, -1))
        test_set = jnp.asarray(K_vec.reshape(1, -1))

        print(f"[PHASE_2_OTF_3D] Starting OTF training: sid={sample_id}")
        t_train0 = time.time()
        ...
        print("[PHASE_2_OTF_3D] Restored best OTF checkpoint from flax_train_state.")
        ...
        # ============================================================
        FNO_UVW = np.array(pi_fno_otf.Predict(K_vec.reshape(1, -1))).reshape(-1)

        export_mesh = create_3D_box_mesh_structured(Nx=Nx, Ny=Ny, Nz=Nz, Lx=L, Ly=L, Lz=L)
        export_mesh.Initialize()
        # Plotter3D discovers matching fields through these standard prefixes.
        # Each case directory contains one OTF sample, so use a local index of 0.
        export_mesh["U_FOL_0"] = FNO_UVW.reshape((n_nodes, 3))
        export_mesh["K_0"] = K_vec.reshape((n_nodes,))
        export_mesh[f"U_FNO_{sample_id}_otf"] = FNO_UVW.reshape((n_nodes, 3))
        export_mesh[f"K_field_{sample_id}_otf"] = K_vec.reshape((n_nodes,))
        ...
        if solve_FE:
            ...
            export_mesh["U_FE_0"] = FE_UVW.reshape((n_nodes, 3))
            export_mesh[f"U_FE_{sample_id}_otf"] = FE_UVW.reshape((n_nodes, 3))
            ...
            plt.title(f"Newton convergence – sample {sample_id} (OTF-3D)")
            ...
            export_mesh[f"abs_error_{sample_id}_otf"] = abs_err_uvw.reshape((n_nodes, 3))
            export_mesh[f"abs_error_ux_{sample_id}_otf"] = abs_err_ux.reshape((n_nodes,))
            export_mesh[f"abs_error_uy_{sample_id}_otf"] = abs_err_uy.reshape((n_nodes,))
            export_mesh[f"abs_error_uz_{sample_id}_otf"] = abs_err_uz.reshape((n_nodes,))
            ...
            _save_2x2_panel(
                ...
                Ny=Ny,
                Nz=Nz,
                out_path=os.path.join(case_dir, f"plot_results_sample_{sample_id}_otf.png"),
            )
        else:
            ...
            for a in axs:
                ...
                a.set_yticks([])
            plt.tight_layout()
            plt.savefig(os.path.join(case_dir, f"plot_results_sample_{sample_id}_otf.png"), dpi=200)
            plt.close()
        ...
        if solve_FE:
            mesh_paths = sorted(
                glob.glob(os.path.join(case_dir, "*.vtu"))
                + glob.glob(os.path.join(case_dir, "*.vtk"))
            )
            if not mesh_paths:
                raise FileNotFoundError(
                    f"No VTK/VTU mesh was exported in {case_dir}; cannot render 3D plots."
                )

            plotter3d = Plotter3D(
                vtk_path=mesh_paths[0],
                warp_factor=1.0,
                config={
                    "clip": False,
                    "output_image": f"plot_results_3d_sample_{sample_id}_otf.png",
                },
            )
            plotter3d.render_all_panels()
        else:
            print("[PHASE_2_OTF_3D] Skipping Plotter3D because solve_FE=False.")

        # ============================================================
        # 9) WRITE STATS (per-sample + master)
        # ============================================================
        row = {
            "phase": "PHASE_2_OTF_3D",
        ...
        print(f"[PHASE_2_OTF_3D] Wrote per-sample stats: {per_sample_csv}")
        print(f"[PHASE_2_OTF_3D] Appended master stats: {master_stats_csv}")

        if clean_dir:
            shutil.rmtree(case_dir)
            print(f"[PHASE_2_OTF_3D] Cleaned directory {case_dir}.")
...
if __name__ == "__main__":
    ...
    out_root = str(script_dir / "PHASE_2_OTF_3D_Fourier")
    ...
    for arg in args:
        if arg.startswith("fol_num_epochs="):
            ...
            print(
                ...
                "  - OTF is run only on test_ids (one fresh model per selected test sample).\n"
