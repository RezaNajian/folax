...
working_directory_name = 'siren_implicit_mechanical_2D_single_sample_free_param'
...
free_param = IdentityControl("free_param_control",{},fe_mesh)

fe_mesh.Initialize()
mechanical_loss_2d.Initialize()
fourier_control.Initialize()
free_param.Initialize()
...
# create fol
fol = ImplicitParametricOperatorLearning(name="dis_fol",control=free_param,
...
fol.Train(train_set=(K_matrix[eval_id,:].reshape(-1,1).T,),batch_size=100,
...
def compute_sens(single_sample_K):
    temp_FOL_UV = jnp.array(fol.Predict(single_sample_K.reshape(-1,1).T)).reshape(-1)
    return jnp.sum(temp_FOL_UV[0::2])

value,grads = jax.value_and_grad(compute_sens,argnums=0)(K_matrix[eval_id,:])
...
plot_mesh_vec_data(1,[FOL_UV[0::2],FOL_UV[1::2],grads],
                   ["U","V","sens"],
