...
import optax
import numpy as np
from fol.loss_functions.thermal_transient import ThermalTransientLoss2DQuad
...
working_directory_name = 'meta_learning_thermal_transient_new_N21_2'
...
fol.Train(train_set=(coeffs_matrix[train_start_id:train_end_id,:],),
          ...
          test_settings={"test_frequency":10},batch_size=16,
...
print("Average relative L2 error:", relative_L2_error/(test_end_id-test_start_id))
...
for test in range(eval_start_id,eval_end_id):
    ...
    plot_mesh_vec_data(1,[coeffs_matrix[eval_id],FOL_T[:,0],FOL_T[:,1],FOL_T[:,-1]],
                ["","","",""],
                fig_title="",cmap = "jet",
                file_name=os.path.join(case_dir,f"para_FOL-dist_test_{eval_id}.png"))
    
    plot_mesh_vec_data(1,[coeffs_matrix[eval_id],FE_T[:,0],FE_T[:,1],FE_T[:,-1]],
                ["","","",""],
                fig_title="",cmap = "jet",
                file_name=os.path.join(case_dir,f"para_FE-dist_test_{eval_id}.png"))
    plot_mesh_vec_data(1,[coeffs_matrix[eval_id],absolute_error[:,0],absolute_error[:,1],absolute_error[:,-1]],
                ["","","",""],
                fig_title="",cmap = "jet",
                file_name=os.path.join(case_dir,f"para_FOL-FE-error-dist_test_{eval_id}.png"))
