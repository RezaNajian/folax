...
def plot_mesh_vec_data_thermal_clear(L, vectors_list, subplot_titles=None, fig_title=None, cmap='viridis',
                       block_bool=False, colour_bar=True, colour_bar_name=None,
                       X_axis_name=None, Y_axis_name=None, show=False, file_name=None):
    num_vectors = len(vectors_list)
    if num_vectors < 1 or num_vectors > 4:
    ...
    for i, squared_mesh_vec_data in enumerate(vectors_list):
        ...
        if colour_bar:
            ...
            cbar.set_ticks(np.linspace(squared_mesh_vec_data.min(), squared_mesh_vec_data.max(), 3))
...
def plot_mesh_quad(points, elements, background=None, filename=None):
    """
...
def plot_dual_triangulated_gif(points, elements, values_list_1, values_list_2, 
                               filename="dual_animation.gif", fps=10, titles=("Solution 1", "Solution 2")):
    """
    Create a side-by-side GIF from two sequences of nodal solutions with a **constant** color scale.
    Parameters:
    points (ndarray): (N, 2) array of node coordinates.
    elements (ndarray): (M, 3) array of triangular element connectivity.
    values_list_1 (ndarray or list of ndarrays): First set of nodal solution fields, shape (num_frames, num_nodes).
    values_list_2 (ndarray or list of ndarrays): Second set of nodal solution fields, shape (num_frames, num_nodes).
    filename (str): Name of the output GIF file.
    fps (int): Frames per second for the GIF.
    titles (tuple of str): Titles for the subplots.
    """
    points = np.asarray(points, dtype=float)
    elements = np.asarray(elements, dtype=int)
    # Convert NumPy arrays to lists if necessary
    if isinstance(values_list_1, np.ndarray):
        values_list_1 = [values_list_1[i, :] for i in range(values_list_1.shape[0])]
    if isinstance(values_list_2, np.ndarray):
        values_list_2 = [values_list_2[i, :] for i in range(values_list_2.shape[0])]
    # Compute global colorbar range (constant for all frames)
    vmin = min(np.min(np.concatenate(values_list_1)), np.min(np.concatenate(values_list_2)))
    vmax = max(np.max(np.concatenate(values_list_1)), np.max(np.concatenate(values_list_2)))
    norm = Normalize(vmin=vmin, vmax=vmax)
    temp_dir = "frames"
    os.makedirs(temp_dir, exist_ok=True)
    filenames = []
    num_frames = min(len(values_list_1), len(values_list_2))  # Ensure both lists have the same length
    for i in range(num_frames):
        fig, axes = plt.subplots(1, 2, figsize=(10, 5))  # Two subplots side by side
        
        for ax, values, title in zip(axes, [values_list_1[i], values_list_2[i]], titles):
            triang = tri.Triangulation(points[:, 0], points[:, 1], elements)
            # Apply **constant** colorbar range
            contour = ax.tricontourf(triang, values, levels=100, cmap="jet", norm=norm)
            ax.set_xticks([])
            ax.set_yticks([])
            ax.set_frame_on(False)
            ax.set_aspect('equal')
            # ax.set_title(title, fontsize=14)
        # Add a **single shared colorbar** for both subplots
