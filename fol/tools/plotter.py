"""
 ...
 A configurable 3D/2D plotter for FOL/iFOL inference results saved in VTK files.
...
class Plotter2D(Plotter3D):
    """
    Flat-mesh visualiser that reuses Plotter3D’s config.

    • Assumes z-extent ≈ 0  (raises ValueError otherwise)
    • Chooses the eval_id with the smallest global L2-norm of abs_error_i
    • Produces one stitched PNG:

          FOL |U|   |   REF |U|   |   |ΔU|

    All colour-maps, scalar-bar args, zoom, etc. come from the same
    `config` dict you already use for Plotter3D.
    """
    ...
    def __init__(self, vtk_path: str, *, config: dict):
        ...
        # ensure this is truly a flat mesh
        if abs(self.mesh.bounds[-1] - self.mesh.bounds[-2]) > 1e-8:
            raise ValueError("Plotter2D expects a flat mesh (z ≈ 0); use Plotter3D instead.")

        self.do_clip   = False                      # never clip a 2-D sheet
        self.u_fol_pre = self.name_map["u_fol_prefix"]
        self.u_ref_pre = self.name_map["u_fe_prefix"]

        self._find_best_sample_by_abs_error()       # sets self.best_id & self.fields
    ...
    def _ensure_mag(self, vec_name: str) -> str:
        """
        Return <vec_name>_mag.  Create it on-the-fly if absent.
        Works for 2- or 3-component point-data arrays.
        """
        arr = self.mesh[vec_name]
        if arr.ndim == 2 and arr.shape[1] in (2, 3):
            mag_name = f"{vec_name}_mag"
            if mag_name not in self.mesh.point_data:
                self.mesh.point_data[mag_name] = np.linalg.norm(arr, axis=1)
            return mag_name
        return vec_name   # already scalar

    def _find_best_sample_by_abs_error(self):
        """
        • Auto-creates abs_error_i_mag if only vectors exist.
        • Picks the eval_id with the smallest global L2 norm.
        """
        # ensure magnitudes exist
        ...
        print(f"[Plotter2D] selected eval_id={best_id}  (‖error‖₂ = {min_err:.4e})")

    # ──────────────────────────────────────────────────────────
    # rendering (overrides only what is different from 3-D)
    # ──────────────────────────────────────────────────────────
    def render_panel(self, mesh_obj, field, clim, title, fname, show_edges=None):
        """
        Same purpose as Plotter3D.render_panel but:
        • forces top-down XY view
        • uses parallel (orthographic) projection
        • hides 3-D axes gizmo for a clean 2-D look
        """
        if show_edges is None:
            show_edges = self.show_edges_default
        ...
        plotter.view_xy()                       # camera straight down
        plotter.enable_parallel_projection()    # no perspective
        plotter.camera.zoom(self.shared_zoom)
        plotter.show_axes = False               # remove 3-D triad
        ...
        plotter.close()
        print(f"[Plotter2D] saved panel → {out_path}")

    # ──────────────────────────────────────────────────────────
    # full 1×3 overview pipeline
    # ──────────────────────────────────────────────────────────
    def render_all_panels(self):
        # field aliases
        fol_vec = self.fields["U_FOL"]
        ref_vec = self.fields["U_REF"]
        err_vec = self.fields["ERR"]

        # make sure scalar magnitudes exist
        fol_mag = self._ensure_mag(fol_vec)
        ref_mag = self._ensure_mag(ref_vec)
        err_mag = self._ensure_mag(err_vec)

        clim_u = [0.0,
                  float(max(self.mesh[fol_mag].max(),
                            self.mesh[ref_mag].max()))]
        clim_e = [0.0, float(self.mesh[err_mag].max())]

        panels = [
            (self.mesh, fol_mag, clim_u,  "FOL |U|",  "panel_fol.png"),
            (self.mesh, ref_mag, clim_u,  "FE |U|",  "panel_ref.png"),
            (self.mesh, err_mag, clim_e,  "abs_error",   "panel_err.png"),
        ]

        for m, f, c, t, fn in panels:
            self.render_panel(m, f, c, t, fn)

        # ── stitch into one overview PNG ───────────────────────
        import matplotlib.pyplot as plt, matplotlib.image as mpimg
        fig, ax = plt.subplots(1, 3, figsize=(18, 6))
        for a, (_, _, _, _, fn) in zip(ax, panels):
            a.imshow(mpimg.imread(os.path.join(self.output_dir, fn)))
            a.axis("off")

        overview_name = self.config.get("output_image", "overview2d.png")
        out = os.path.join(self.output_dir, overview_name)
        plt.tight_layout()
