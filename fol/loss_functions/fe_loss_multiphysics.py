...
class FiniteElementLoss(Loss):
    ...
    def __CreateDofsDict(self, dofs_list:list, dirichlet_bc_dict:dict):
        ...
        for dof_index,dof in enumerate(dofs_list):
            ...
            for boundary_name,boundary_value in dirichlet_bc_dict[dof].items():
                ...
                if dof in self.first_dofs:
                    dirichlet_bc_indices_1 = number_dofs_per_node*boundary_node_ids + dof_index
                    dirichlet_indices_1.extend(dirichlet_bc_indices_1.tolist())

                elif dof in self.second_dofs:
                    dirichlet_bc_indices_2 = number_dofs_per_node*boundary_node_ids + dof_index - self.number_first_dofs_per_node
                    dirichlet_indices_2.extend(dirichlet_bc_indices_2.tolist()) 
        ...
        self.first_dofs_dirichlet_indices = jnp.array(dirichlet_indices_1)
        self.second_dofs_dirichlet_indices = jnp.array(dirichlet_indices_2)

        # self.first_dofs_dirichlet_indices = jnp.array([full_dof_indices_dict[dof] for dof in self.first_dofs])
    ...
    def __CreateDofsDictFE(self, first_dofs_list:list, second_dofs_list:list, dirichlet_bc_dict:dict):
        ...
        self.first_dofs_dirichlet_indicesFE = jnp.array(dirichlet_indices)
        self.first_dofs_dirichlet_valuesFE = jnp.array(dirichlet_values)
        second_dirichlet_indices = []
        second_dirichlet_values = []
        for dof_index,dof in enumerate(second_dofs_list):  
            ...
            for boundary_name,boundary_value in dirichlet_bc_dict[dof].items():
                ...
                boundary_start_index = len(dirichlet_indices)
                second_dirichlet_indices.extend(dirichlet_bc_indices.tolist())
                boundary_end_index = len(dirichlet_indices)

                dirichlet_dofs_boundary_dict[dof][boundary_name] = jnp.arange(boundary_start_index,boundary_end_index)

                dirichlet_bc_values = boundary_value * jnp.ones(dirichlet_bc_indices.size)
                second_dirichlet_values.extend(dirichlet_bc_values.tolist())

        dirichlet_indices.extend(second_dirichlet_indices)
        dirichlet_values.extend(second_dirichlet_values)
        
        self.second_dofs_dirichlet_indicesFE = jnp.array(second_dirichlet_indices)
        self.second_dofs_dirichlet_valuesFE = jnp.array(second_dirichlet_values)
    ...
    @print_with_timestamp_and_execution_time
    @partial(jit, static_argnums=(0,2,))
    def ApplyDirichletBCOnDofVectorMulti(self,full_dof_vector:jnp.array,load_increment:float=1.0):
        # return full_dof_vector.at[self.second_dofs_dirichlet_indicesFE].set(load_increment*self.second_dofs_dirichlet_valuesFE)
        return full_dof_vector.at[self.dirichlet_indicesFE].set(load_increment*self.dirichlet_valuesFE)
    ...
    def ComputeSingleLossStaggered(self,full_control_param:jnp.array,unknown_dofs:jnp.array):
        ...
        phy1_elem_energy = jax.lax.stop_gradient((jnp.abs(elems_energies_1)))
        phy2_elem_energy = jax.lax.stop_gradient((jnp.abs(elems_energies_2)))
        return elems_energies, elems_energies_1, elems_energies_2,(min_elem_energy,max_elem_energy,avg_elem_energy,phy1_elem_energy,phy2_elem_energy)
    ...
    def ComputeJacobianMatrixAndResidualVector(self,total_control_vars: jnp.array,total_primal_vars: jnp.array, transpose_jacobian:bool=False):
        total_control_vars = self.GetFullControlVector(total_control_vars)
