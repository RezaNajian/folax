...
class PhaseFieldLoss(FiniteElementLossPhasefield):
    ...
    def ComputeElement(self,xyze,Te_c,Te_n,body_force=0):
        ...
        def compute_at_gauss_point(gp_point,gp_weight):
            ...
            gp_t = 0.5/(self.dt)*gp_weight  * detJ *(T_at_gauss_n-T_at_gauss_c)**2
            gp_Df = jnp.outer(N_vec, N_vec) * (3 * T_at_gauss_n**2 - 1) *  detJ * gp_weight
        ...
        return 0.5*Te_n.T@Se@Te_n + 1/(self.epsilon**2)*Fe + Te, ((Me+self.dt*Se)@Te_n - (Me@Te_c- 1/(self.epsilon**2)*self.dt*Fe_res)), (Me+self.dt*Se - self.dt/(self.epsilon**2)*dFe)


class PhaseFieldLoss2DQuad(PhaseFieldLoss):
