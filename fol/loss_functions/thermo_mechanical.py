...
class ThermoMechanicalLoss(FiniteElementLoss):
    ...
    def Initialize(self,reinitialize=False) -> None:  
        ...
        if "T0" in self.loss_settings["material_dict"].keys():
            self.T0 = self.loss_settings["material_dict"]["T0"]
        else:
            self.T0 = jnp.zeros((self.fe_mesh.GetNumberOfNodes(),))
    ...
    def ComputeElement_1(self,xyze,de,te,se,te_init,body_force=0):
        ...
        def compute_at_gauss_point(gp_point,gp_weight):
            ...
            conductivity_at_gauss = jnp.dot(N_vec.reshape(1,-1), de) #* (1 + self.thermal_loss_settings["beta"]*(jnp.dot(N,te.squeeze()))**self.thermal_loss_settings["c"])
            ...
            return gp_loss_t, gp_stiffness,gp_f
        gp_points,gp_weights = self.fe_element.GetIntegrationData()
        loss_t_gps,s_gps, f_gps = jax.vmap(compute_at_gauss_point,in_axes=(0,0))(gp_points,gp_weights)
        ...
        return  Loss_t_e, element_residuals, Se, Se_s
    ...
    def ComputeElement_2(self,xyze,de,te,se,te_init,body_force=0):
        ...
        def compute_at_gauss_point(gp_point,gp_weight):
            ...
            thermal_strain_vec = self.thermal_loss_settings["alpha"] * (temp_at_gauss - jnp.dot(N_vec, te_init.squeeze())) * self.thermal_st_vec
            ...
            gp_stiffness_thermal = - gp_weight * detJ *  e_at_gauss * self.thermal_loss_settings["alpha"] * (B_mat.T @ self.D @ self.thermal_st_vec).reshape(-1,1) @ N_vec.reshape(1,-1)
        ...
        # element_residuals = compute_elem_res(Se,se ,Fe, Fe_thermal)
        element_residuals = LHSe - Fe
        return  Loss_m_e, element_residuals, Se, Se_t
    ...
    def ComputeElementResidualAndJacobianVmapCompatible(self,element_id:jnp.integer,
        ...
        return self.ComputeElementResidualAndJacobian(xyz[elements_nodes[element_id],:],
                                                      ...
                                                      full_dof_vector_2[((self.number_second_dofs_per_node*elements_nodes[element_id])[:, jnp.newaxis] +
                                                      jnp.arange(self.number_second_dofs_per_node))].reshape(-1,1),
                                                      self.T0[elements_nodes[element_id]],
...
class ThermoMechanicalLoss3DTetra(ThermoMechanicalLoss):
    @print_with_timestamp_and_execution_time
...
class ThermoMechanicalLoss3DHexa(ThermoMechanicalLoss):
    @print_with_timestamp_and_execution_time
    def __init__(self, name: str, loss_settings: dict, fe_mesh: Mesh):
...
class ThermoMechanicalLoss2DQuad(ThermoMechanicalLoss):
    def __init__(self, name: str, loss_settings: dict, fe_mesh: Mesh):
        if not "num_gp" in loss_settings.keys():
            loss_settings["num_gp"] = 2
...
class ThermoMechanicalLoss2DTri(ThermoMechanicalLoss):
    def __init__(self, name: str, loss_settings: dict, fe_mesh: Mesh):
        super().__init__(name,{**loss_settings,"compute_dims":2,
                               "ordered_dofs": ["T", "Ux","Uy"],
                               "first_dofs": ["T"], 
