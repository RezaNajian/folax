...
class ThermalTransientLossNonlinearHetero(FiniteElementLossTransientHetero):
    ...
    def Initialize(self) -> None:  
        ...
        if self.dim == 2:
            ...
            self.alpha_k =  self.loss_settings["material_dict"]["alpha_k"]
    ...
    def ComputeElementHetero(self,xyze,Te_c,Ke,Te_n,body_force=0):
        ...
        def compute_at_gauss_point(gp_point,gp_weight):
            ...
            gp_stiffness = B_mat.T @ (ke_at_gauss * B_mat) * detJ * gp_weight * (1 + self.alpha_k * T_at_gauss_n)
            gp_mass = self.rho * self.cp* jnp.outer(N_vec, N_vec) * detJ * gp_weight 
            gp_f = gp_weight * detJ * N_vec.reshape(-1,1) #* body_force
            gp_t = self.rho * self.cp * 0.5/(self.dt)*gp_weight  * detJ *(T_at_gauss_n-T_at_gauss_c)**2
            gp_dR = (ke_at_gauss + self.alpha_k * ke_at_gauss * N_vec.reshape(-1, 1)) * B_mat.T @ B_mat * detJ * gp_weight
            return gp_stiffness,gp_mass, gp_f, gp_t, gp_dR

        gp_points,gp_weights = self.fe_element.GetIntegrationData()
        k_gps,m_gps,f_gps,t_gps, dR_gps = jax.vmap(compute_at_gauss_point,in_axes=(0,0))(gp_points,gp_weights)
...
class ThermalTransientLossNonlinear2DQuad(ThermalTransientLossNonlinearHetero):
...
class ThermalTransientLossNonlinear2DTri(ThermalTransientLossNonlinearHetero):
    def __init__(self, name: str, loss_settings: dict, fe_mesh: Mesh):
        super().__init__(name,{**loss_settings,"compute_dims":2,
                               "ordered_dofs": ["T"],  
                               "element_type":"triangle"},fe_mesh)

class ThermalTransientLossNonlinear3DHexa(ThermalTransientLossNonlinearHetero):
    def __init__(self, name: str, loss_settings: dict, fe_mesh: Mesh):
        if not "num_gp" in loss_settings.keys():
            loss_settings["num_gp"] = 2
