...
class ThermalTransientLoss(FiniteElementLoss):
...
class ThermalTransientLoss2DQuad(ThermalTransientLoss):
...
class ThermalTransientLoss3DHexa(ThermalTransientLoss):
    def __init__(self, name: str, loss_settings: dict, fe_mesh: Mesh):
        if not "num_gp" in loss_settings.keys():
            loss_settings["num_gp"] = 2
