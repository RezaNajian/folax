...
from .deep_network_super_res import DeepNetwork
...
class ImplicitParametricOperatorLearning(DeepNetwork):
    ...
    def __init__(self,
                 ...
                 control:Control,
                 loss_function:Loss,
                 loss_function_pred:Loss,
                 flax_neural_network:nnx.Module,
                 optax_optimizer:GradientTransformation,
                 checkpoint_settings:dict={},
                 working_directory='.'
                 ):
        super().__init__(name,loss_function,loss_function_pred,
                         flax_neural_network,
    ...
    def ComputeSingleLossValue(self,x_set:Tuple[jnp.ndarray, jnp.ndarray], nn_model:nnx.Module):
        ...
        hetero_output = self.control.ComputeControlledVariables(x_set[1][1:])
        return self.loss_function.ComputeSingleLossHetero(control_output,hetero_output,nn_output)
    ...
    def Predict_fine(self,batch_X):
        ...
        prediction = []
        for i in range(batch_X.shape[0]):
            tiled_input = jnp.hstack((jnp.tile(batch_X, (self.loss_function_pred.fe_mesh.GetNodesCoordinates().shape[0], 1)),self.loss_function_pred.fe_mesh.GetNodesCoordinates()))
            # nn_output = self.flax_neural_network(self.loss_function.fe_mesh.GetNodesCoordinates()).flatten()[self.loss_function.non_dirichlet_indices]
            nn_output = self.flax_neural_network(tiled_input).flatten()[self.loss_function_pred.non_dirichlet_indices]
            full_dof = self.loss_function_pred.GetFullDofVector(batch_X[i],nn_output)
            prediction.append(full_dof)
        return jnp.array(prediction)
