...
class FiniteElementNonLinearResidualBasedSolverHetero(FiniteElementSolver):
    ...
    @print_with_timestamp_and_execution_time
    def Solve(self,current_control_vars:jnp.array,current_hetero_vars:jnp.array,current_dofs:jnp.array):
        ...
        for load_fac in range(load_increament):
            ...
            for i in range(self.nonlinear_solver_settings["maxiter"]):
                BC_applied_jac,BC_applied_r = self.fe_loss_function.ComputeJacobianMatrixAndResidualVectorHetero(
                                                                    current_control_vars,current_hetero_vars,applied_BC_dofs)
